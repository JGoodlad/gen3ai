//! MOVE-COVERAGE BATCH 4b full-battle test (`gen3_move_coverage_batch4b_v1`) — the per-seed
//! PER-DECISION STATE(+HP+STATUS+BOOSTS+SUB-HP)+SEED+winner differential that proves the THREE
//! remaining MISMODELED single-turn damaging moves match Showdown EXACTLY, to GAME-END:
//!
//!   * **BEAT UP** — a MULTI-STRIKE move: ONE strike PER healthy (non-fainted, NON-STATUSED)
//!     party member of the USER's side, each a TYPELESS flat-BP-10 Special hit with the STAT
//!     SWAP (ally dex base-atk → attacker SpA, target dex base-def → defender SpD, modifier=1 —
//!     no boosts/items/CB/burn/abilities). ONE accuracy roll (acc 100) draws BEFORE the loop,
//!     then PER STRIKE crit `randomChance(1,16)` + damage `random(16)`. The multihit STOPS at
//!     the target's faint (later strikes + Quick Claw skip); a statused/fainted teammate (incl.
//!     a statused ACTIVE user) skips its strike; a sub break lets later strikes hit the mon.
//!     Proved by the per-strike HP + the SEED (the per-strike crit+damage draw pair).
//!   * **THUNDER** — a 120-BP Special Electric move (base acc 70, 30% para) whose id-gated
//!     onModifyMove REWRITES the base accuracy by the target's effective weather: RAIN →
//!     never-miss (the accuracy `random(100)` is SKIPPED — ONE FEWER draw), SUN → base 50, else
//!     (none / sand / hail / Cloud-Nine-or-Air-Lock-suppressed) → base 70. Proved by the SEED
//!     (rain removes EXACTLY the accuracy draw) + the target HP (the hit/miss outcome).
//!   * **WATER SPOUT** — a variable-BP Special Water move: `bp = max(floor(150·hp/maxhp), 1)`,
//!     a deterministic STATE read of the USER's CURRENT hp, DRAW-NEUTRAL (only the damage
//!     magnitude changes). Proved by the target HP (the variable damage) at the SAME SEED.
//!
//! Each move's draws must be in Showdown's EXACT place/count — a stray/missing draw desyncs the
//! LCG at the SEED assertion; the STATE columns are the effect proof. The golden REUSES the
//! batch-3/4 TAB format (per-side CURSE + WISH-PENDING + SUB-HP columns → DEC has 42 fields):
//! CURSE + WISH-PENDING are always 0 (asserted so — no spurious curse/wish), SUB-HP is live
//! (Beat Up / Water Spout into a sub).
...
/// `slot` targets a specific PARTY index (default 0 = the active lead) — Beat Up's
/// statused-teammate scenario injects a status onto a BENCH slot (whose strike is then
/// skipped, a draw-count effect).
#[derive(Debug, Clone, Default)]
struct Inject {
    side: Option<usize>,
    slot: Option<usize>,
...
fn parse_golden() -> (BTreeMap<String, ScenMeta>, Vec<RunCase>) {
    let path = concat!(env!("CARGO_MANIFEST_DIR"), "/tests/vectors/movecoverage_batch4b_golden.txt");
    let data = std::fs::read_to_string(path).unwrap_or_else(|e| {
        panic!(
            "missing batch4b golden ({path}): {e}\nrun: node src/rust_sim/harness/gen_movecoverage_batch4b_golden.js"
        )
    });
...
fn apply_inject(battle: &mut Battle, inject: &[Inject]) {
    ...
    for inj in inject {
        if let Some(side) = inj.side {
            // `slot` defaults to the active lead (index 0 at construction). A Beat Up
            // statused-teammate inject targets a BENCH slot directly.
...
#[test]
fn movecoverage_batch4b_golden_matches_showdown() {
    let d = dex();
    ...
    for case in &cases {
        ...
        for (di, (rec, exp)) in outcome.decisions.iter().zip(case.decisions.iter()).enumerate() {
            ...
            // --- PER-DECISION SEED PARITY (the draw-order+count proof). BEAT UP draws ONE
            //     accuracy roll then PER STRIKE crit+damage (stopping at the target's faint);
            //     THUNDER in rain SKIPS the accuracy draw (never-miss) while sun/base draw it;
            //     WATER SPOUT is draw-neutral (the variable BP is a state read). A stray/
            //     missing/mis-ordered draw desyncs HERE. ---
            assert_eq!(
                rec.seed_after, exp.seed_after,
                "[{}] dec {} SEED mismatch (init_seed {}): got {} exp {}\n  \
                 a batch-4b move drew (or skipped) a PRNG call — Beat Up's per-strike crit+damage \
                 pair, Thunder's rain-never-miss (one fewer accuracy draw), or Water Spout's \
                 draw-neutral variable BP is wrong. FIX THE DRAW MODEL.",
    ...
    assert_eq!(curse_rows, 0, "BEAT UP / THUNDER / WATER SPOUT must never set the curse volatile, got {curse_rows}");
    assert_eq!(wish_rows, 0, "BEAT UP / THUNDER / WATER SPOUT must never leave a Wish pending, got {wish_rows}");
    assert!(sub_rows >= 50, "expected substitute-up rows (>=50 — Beat Up / Water Spout into a sub), got {sub_rows}");
    assert!(win_runs >= 40, "expected real game-end WIN runs (>=40), got {win_runs}");
    let _ = boost_rows;

    eprintln!(
        "movecoverage batch4b golden: {} runs, {dec_assertions} STATE rows, {seed_assertions} seed assertions, \
