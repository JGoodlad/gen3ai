//! MOVE-COVERAGE BATCH 3 full-battle test (`gen3_move_coverage_batch3_v1`) — the per-seed
//! PER-DECISION STATE(+HP+STATUS+BOOSTS+CURSE+WISH-PENDING+SUB-HP)+SEED+winner differential
//! that proves the three STATEFUL, DRAW-FREE move classes match Showdown EXACTLY, to GAME-END:
//!
//!   * **CURSE** — a type-conditional move. NON-GHOST → a self-boost {atk:+1, def:+1, spe:-1}
//!     (the mixed +/- setup; the -Spe can flip the first mover next turn). Its `move.self`
//!     rides the gen3 `selfDrops` path, which DRAWS ONE `random(100)` (discarded) — so it is
//!     NOT draw-free (like Overheat's self-drop). GHOST → pays floor(maxhp/2) HP + lays the
//!     `curse` volatile on the FOE (residual
//!     chip floor(maxhp/4)/turn at order 10 subOrder 8). Re-curse fails; curse-into-a-sub does
//!     nothing; a Ghost target is NOT immune. Proved by the CURSE columns + HP + the SEED.
//!   * **WISH** — a slot-keyed DELAYED heal: floor(maxhp/2) at the END of the turn AFTER cast
//!     (duration 2). Slot-keyed (survives switch). Residual ORDER 7 (BEFORE the sand chip +
//!     order-10 handlers); two Wishes at equal speed tie-shuffle. Double-Wish FAILS; heal-at-
//!     full is silent. Proved by the WISH-PENDING column + HP + the SEED.
//!   * **BATON PASS** — a self-switch passing the outgoing mon's boosts + copyable volatiles
//!     (substitute / leech-seed / confusion / curse) to the entrant. No bench → FAILS. Proved
//!     by the post-switch BOOST/SUB-HP/CURSE columns + the SEED.
//!
//! The non-ghost curse draws ONE `random(100)` (the selfDrops roll); the rest (ghost curse,
//! wish, baton pass) are draw-free beyond the existing shuffles (a wish-mirror at equal speed
//! draws one tie-shuffle). So the SEED parity is the draw-model proof (a stray/missing draw
//! desyncs the LCG) and the STATE columns are the effect proof. The golden TAB format extends the
//! secondary one: it drops the batch-2 weather/screen tail and appends per-side CURSE +
//! WISH-PENDING + SUB-HP columns → DEC has 41 fields, plus an INJECT line per scenario.
...
fn parse_golden() -> (BTreeMap<String, ScenMeta>, Vec<RunCase>) {
    let path = concat!(env!("CARGO_MANIFEST_DIR"), "/tests/vectors/movecoverage_batch3_golden.txt");
    let data = std::fs::read_to_string(path).unwrap_or_else(|e| {
        panic!(
            "missing batch3 golden ({path}): {e}\nrun: node src/rust_sim/harness/gen_movecoverage_batch3_golden.js"
        )
    });
...
#[test]
fn movecoverage_batch3_golden_matches_showdown() {
    let d = dex();
    ...
    assert!(curse_rows >= 50, "expected cursed rows (>=50), got {curse_rows}");
    assert!(wish_rows >= 50, "expected wish-pending rows (>=50), got {wish_rows}");
    assert!(sub_rows >= 50, "expected substitute-up rows (>=50), got {sub_rows}");
    assert!(boost_rows >= 50, "expected boosted rows (>=50), got {boost_rows}");
    assert!(win_runs >= 40, "expected real game-end WIN runs (>=40), got {win_runs}");

    eprintln!(
        "movecoverage batch3 golden: {} runs, {dec_assertions} STATE rows, {seed_assertions} seed assertions, \
