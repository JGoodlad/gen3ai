//! MOVE-COVERAGE BATCH 6 full-battle test (`gen3_move_coverage_batch6_v1`) — the per-seed
//! PER-DECISION STATE(+HP+STATUS+BOOSTS+SUB-HP+ENCORE+PERISH+TRAPPED)+SEED+winner
//! differential that proves the FINAL UNMODELED tail matches Showdown EXACTLY, to
//! GAME-END:
//!
...
fn parse_golden() -> (BTreeMap<String, ScenMeta>, Vec<RunCase>) {
    let path = concat!(env!("CARGO_MANIFEST_DIR"), "/tests/vectors/movecoverage_batch6_golden.txt");
...
#[test]
fn movecoverage_batch6_golden_matches_showdown() {
    let d = dex();
    ...
    assert_eq!(curse_rows, 0, "BATCH 6 must never set the curse volatile, got {curse_rows}");
    assert_eq!(wish_rows, 0, "BATCH 6 must never set a wish, got {wish_rows}");
    assert_eq!(future_rows, 0, "BATCH 6 must never set a future move, got {future_rows}");
    assert!(sub_rows >= 50, "expected substitute-up rows (>=50), got {sub_rows}");
    assert!(encore_rows >= 200, "expected encore-up rows (>=200), got {encore_rows}");
    assert!(perish_rows >= 200, "expected perish-counter rows (>=200), got {perish_rows}");
    assert!(trapped_rows >= 200, "expected FIRM-trapped Move rows (>=200), got {trapped_rows}");
    assert!(win_runs >= 40, "expected real game-end WIN runs (>=40), got {win_runs}");

    eprintln!(
        "movecoverage batch6 golden: {} runs, {dec_assertions} STATE rows, {seed_assertions} seed assertions, \
