//! ACCURACY pipeline class-sweep tests (`gen3_accuracy_pipeline_v1`) — the per-seed
//! PER-DECISION STATE+HP+SEED+winner differential proving the DATA-DRIVEN to-hit
//! computation matches Showdown EXACTLY, to GAME-END, for the acc/eva stage table + every
//! wired accMod item/ability member.
//!
//! **This phase is DRAW-RELEVANT** (unlike the P1/P2 stat/BP folds): a wrong effective
//! accuracy → a hit/miss FLIP → the accuracy `randomChance` is drawn, then crit/damage
//! draws follow ONLY on a hit, so the SEED diverges. The `seed_after` assertion below is
//! the teeth — the harshest failure mode.
//!
//!   STAGE FOLD — Muddy Water (100%/30% accuracy-drop secondary) lowers the FOE's accuracy
//!     each hit; the foe's own attacks then roll at 80×(3/4)^n, missing more (the fuzzer's
//!     acc-stage cluster). Reached NATURALLY: the drop stacks over the battle.
//!   ACCURACY_ITEM — Bright Powder (×0.9) / Lax Incense (×0.95) on the DEFENDER.
//!   ACCURACY ability — Compound Eyes (×1.3 attacker), Sand Veil (×0.8 defender in sand),
//!     Hustle (×0.8 attacker, physical-TYPE move).
//!   No-mod CONTROL — the same shaky move with no item/ability/stage (the empty path,
//!     byte-identical to the pre-pipeline raw roll — 0 accMod rows).
//!
//! The golden (`harness/gen_accuracy_golden.js`) drives the OMNISCIENT BattleStream to
...
//! seed + the final winner. The exact effAcc math is additionally pinned at the unit level
//! by `turn.rs::effective_accuracy_matches_sim_probe`. THIS gate proves the engine folds
//! the right acc/eva stages + accMod handlers on every real decision path AND consumes the
//! ONE to-hit draw with the correct probability (a mis-computed effAcc flips a roll and
...
struct DecExpect {
    ...
    p2: SideExpect,
    first_mover: String,
    /// A `-miss` occurred this decision (the stage/mod flipped a roll).
    missed: bool,
    /// A `ModifyAccuracy` handler changed the accuracy this decision (an accMod fired).
    acc_mod: bool,
}
...
fn parse_golden() -> (BTreeMap<String, ScenMeta>, Vec<RunCase>) {
    let path = concat!(env!("CARGO_MANIFEST_DIR"), "/tests/vectors/accuracy_golden.txt");
    let data = std::fs::read_to_string(path).unwrap_or_else(|e| {
        panic!("missing accuracy golden ({path}): {e}\nrun: node src/rust_sim/harness/gen_accuracy_golden.js")
    });
    ...
    for (i, line) in data.lines().enumerate() {
        ...
        match f[0] {
            ...
            "DEC" => {
                ...
                //   p2(...)[15..21)  first[21]  miss[22]  accMod[23]
                assert_eq!(f.len(), 24, "DEC needs 24 fields (line {ln}), got {}", f.len());
                let req = match f[3] {
                    "move" => ReqTok::Move,
...
#[test]
fn accuracy_golden_matches_showdown() {
    let d = dex();
    let (meta, cases) = parse_golden();
    assert!(
        meta.len() >= 7,
        "expected >=7 scenarios (stage fold + every accMod member + control), got {}",
        meta.len()
    );
    assert!(cases.len() >= 180, "expected the per-seed corpus (>=180 runs), got {}", cases.len());

    let mut dec_assertions = 0usize;
    let mut seed_assertions = 0usize;
    let mut miss_rows = 0usize;
    let mut acc_mod_rows = 0usize;
    let mut miss_per_scen: BTreeMap<String, usize> = BTreeMap::new();
    let mut accmod_per_scen: BTreeMap<String, usize> = BTreeMap::new();
    ...
    for case in &cases {
        ...
        for (di, (rec, exp)) in outcome.decisions.iter().zip(case.decisions.iter()).enumerate() {
            ...
            for (idx, (snap, e, sp)) in [
                ...
                // --- THE ACCURACY GATE: a wrong effAcc flips a hit↔miss, so a hit that the
                //     sim landed (or missed) lands a different HP here. The stage table,
                //     accMod multiplier, mod ORDER, and the runEvent integer-guard all
                //     bottom out in this HP + the seed below. ---
                assert_eq!(
                    snap.hp, e.hp,
                    "[{}] dec {} side {} HP mismatch (init_seed {}): got {} exp {}\n  \
                     the effective accuracy is wrong (stage table / accMod multiplier / mod \
                     order / integer-guard) — a hit/miss flipped. Check effective_accuracy + \
                     ItemData/AbilityData.acc_mod.",
            ...
            // --- PER-DECISION SEED PARITY (THE TEETH): the to-hit roll draws exactly ONE
            //     `random(100)`. A wrong effAcc flips the roll → crit/damage draws follow
            //     only on a hit → the LCG desyncs HERE. The stage/mod math is draw-neutral
            //     (still exactly one accuracy draw), so ANY seed drift is a real to-hit bug. ---
            assert_eq!(
                rec.seed_after, exp.seed_after,
                "[{}] dec {} SEED mismatch (init_seed {}): got {} exp {}\n  \
                 a hit/miss FLIPPED (wrong effective accuracy) — the draw stream desynced. \
                 FIX THE effAcc MATH / DRAW ORDER, do not loosen the assert.",
            ...
            if exp.missed {
                miss_rows += 1;
                *miss_per_scen.entry(case.scen.clone()).or_default() += 1;
            }
            if exp.acc_mod {
                acc_mod_rows += 1;
                *accmod_per_scen.entry(case.scen.clone()).or_default() += 1;
            }
        }
    ...
    // Coverage floors keyed on each scenario's declared marker (mirrors the generator):
    //   *stage* scenarios must produce MISSES (the stage fold flips rolls);
    //   accMod scenarios must FIRE the accMod handler;
    //   the *control* must fire NO accMod (the empty path is byte-identical).
    let miss_n = |s: &str| miss_per_scen.get(s).copied().unwrap_or(0);
    let accmod_n = |s: &str| accmod_per_scen.get(s).copied().unwrap_or(0);
    for scen in meta.keys() {
        if scen.contains("stage") {
            assert!(miss_n(scen) >= 10, "[{scen}] only {} miss rows (<10) — the stage fold barely flips rolls", miss_n(scen));
        } else if scen.contains("control") || scen.contains("nomod") {
            assert_eq!(accmod_n(scen), 0, "[{scen}] control fired {} accMod rows (>0) — the empty path must not modify accuracy", accmod_n(scen));
        } else {
            assert!(accmod_n(scen) >= 10, "[{scen}] only {} accMod rows (<10) — the acc item/ability fold barely fires", accmod_n(scen));
        }
    }
    assert!(seed_assertions >= 600, "expected the per-decision seed corpus (>=600), got {seed_assertions}");
    assert!(acc_mod_rows >= 100, "expected accMod rows (>=100), got {acc_mod_rows}");
    assert!(win_runs >= 150, "expected real game-end WIN runs (>=150), got {win_runs}");

    eprintln!(
        "accuracy golden: {} runs over {} scenarios, {dec_assertions} STATE rows, \
         {seed_assertions} seed assertions, {miss_rows} miss rows, {acc_mod_rows} accMod rows, \
