...
- **Pressure −2.** A move TARGETING a **Pressure** holder deducts **2** PP (the `runEvent('DeductPP')`
  extra, battle-actions.ts:472-483), DRAW-FREE, and only for a FOE-targeting move (a self-target
  setup/heal/protect deducts 1). (CLAUDE.md previously called Pressure a "provable no-op in a
