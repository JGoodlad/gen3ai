...
function classifyDamaging(m, id) {
  ...
  // generic mutate-reject below — otherwise the census reports a move the engine models.
  if (MODELED_WEATHERBALL.has(id)) return { cov: 'MODELED', mech: 'weather-ball (gen3_weather_ball_v1)' };
...
function classifyStatus(m, id) {
  ...
  // FOCUS ENERGY (`gen3_focus_energy_move_v1`, ROUND 59) — never-miss, DRAW-FREE, sets the
  // `focusenergy` volatile (+2 crit stages) the port has modelled since the Lansat Berry.
  if (id === 'focusenergy') return { cov: 'MODELED', mech: 'crit-stage volatile (gen3_focus_energy_move_v1)' };
