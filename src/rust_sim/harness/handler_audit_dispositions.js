...
add([mv('uproar', 'self'), cond('uproar', 'onStart'), cond('uproar', 'durationCallback'),
  ...
  IMPL('turn/moves.rs::run_move', 'the lock: `MonState::uproar = Some((turns, slot))`, armed on the CAST turn AFTER the damage with a single random(2,6) that is NEVER re-drawn. It folds into `move_locked()`, which gives the FIRM single-move `trapped:true` request shape, the Move(0)->locked-slot mapping at queue-build, and the switch reject, all for free. PP is spent ONCE per CAST — every continuing turn is a lockedmove that skips deductPP, exactly like Solar Beam\'s fire turn. A cast that did NOT land (miss / type-immune Ghost / Protect) applies no volatile and draws no duration, though its PP is spent.'));
add([cond('uproar', 'onResidual'), cond('uproar', 'onResidualOrder'), cond('uproar', 'onResidualSubOrder'),
     cond('uproar', 'onEnd')],
  IMPL('turn/residuals.rs::run_residuals', 'ResidualAction::UproarDuration at order 10 / subOrder 11 (probe-captured, between the status DoT at 10/6 and Taunt at 10/15), speed = the holder\'s cached_speed. A LIVE tick emits |-start|<u>|Uproar|[upkeep]; the tick that reaches 0 emits |-end|<u>|Uproar and takes the duration-END continue — so, unlike outrage/thrash, there is NO end-of-lock confusion. The lock also clears SILENTLY on switch-out and faint (a phazed-out uproarer emits no -end).'));
add([cond('uproar', 'onAnySetStatus')],
  IMPL('turn/status.rs::try_set_status_impl', 'the FIELD-WIDE sleep block at (2b1): onAnySetStatus, so ONE live uproar on EITHER side blocks sleep for EVERY mon (hence the any_uproarer field scan). DRAW-FREE, and in gen3ou the handler-sort shuffle still draws exactly ONE call — uproar has a DEFINED speed so it takes index 0 and the two Standard clauses stay a size-2 tie, random(1,3) vs the control random(0,2), the same COUNT and therefore seed-neutral. The sleep MOVE still draws its accuracy roll; the sleep random(2,6) does not. Rest is fully blocked (no heal, no random(2,6)). Emits |-fail|<mon>|slp|[from] Uproar, with a trailing |[msg] only when the blocked mon IS the uproarer.'));
add([mv('uproar', 'onTryHit')],
  IMPL('turn/moves.rs::run_move', 'the WAKE: a LANDED uproar cures slp on BOTH ACTIVES, DRAW-FREE, emitted as a bare |-curestatus|<mon>|slp|[msg] with no [from] clause. NOT on a miss, NOT on a type-immune target, and NEVER for a benched sleeper — the first two return before the site, and the loop reads only the two actives.'));

// WEATHER BALL (`gen3_weather_ball_v1`).
add([mv('weatherball', 'onModifyType'), mv('weatherball', 'onModifyMove')],
  IMPL('turn/moves.rs::run_move', 'the weatherball id-gate: reads effective_weather() and rewrites TYPE + BASE POWER + CATEGORY before the crit/damage draws, so it is DRAW-NEUTRAL. rain->Water, sun->Fire, sandstorm->Rock, hail->Ice, all at bp 100; no effective weather leaves the dex row (Normal / 50 / Physical). The CATEGORY is re-derived through the shared derive_category so the gen-3 type split produces it rather than a second id-list — Rock is PHYSICAL while Water/Fire/Ice are SPECIAL. Because it reads effective_weather, a Cloud Nine / Air Lock holder on either side reverts it fully. Pinned by WB2/WB3 via Counter-vs-Mirror-Coat, which sees the category flip even on a board where Atk == SpA.'));

// CONVERSION / CONVERSION 2 (`gen3_conversion_v1`).
add([mv('conversion', 'onHit'), mv('conversion', 'neverMiss'), mv('conversion', 'ignoreImmunity')],
  IMPL('turn/status_moves.rs::run_status_move', 'the conversion arm: never-miss so no accuracy roll; the candidate list is the USER\'s LIVE move slots IN SLOT ORDER keeping move.type for every slot whose id != "curse" and whose type the user does not CURRENTLY have, DUPLICATES KEPT. Exactly ONE random(n) on success (n==1 still draws) and ZERO draws when the list is empty ([still] + a bare -fail on the USER). setType replaces the whole array with one type via types_override, which already reverts on switch-out for Color Change. LIVE matters three ways: a Mimic-overwritten slot contributes the COPIED type, a Color-Changed type excludes its own move, and a second Conversion sees the first result (which is how Conversion\'s own Normal slot enters the list once the user is no longer Normal).'));
add([mv('conversion2', 'onHit'), mv('conversion2', 'neverMiss'), mv('conversion2', 'ignoreImmunity')],
  IMPL('turn.rs::conversion2_candidates', 'keys off the TARGET\'s lastMoveUsed (MonState::last_move_used, reset on switch-out) and type-changes the SOURCE. The list is every type whose damageTaken[attackType] is resist or immune, built in CONVERSION2_TYPE_ORDER — the typedex FILE key order, neither alphabetical nor the port Type enum order — because the single random(n) indexes into it. No lastMoveUsed, or a typeless ??? last move (Curse), gives an empty list and a draw-free [still]+-fail. Extracted as a pure fn so the order is pinned seed-free against the sim\'s own dex dump (all 17 rows).'));

// SAFEGUARD (`gen3_safeguard_v1`).
add([mv('safeguard', 'sideCondition'), mv('safeguard', 'neverMiss'), mv('safeguard', 'ignoreImmunity'),
     cond('safeguard', 'onSideStart'), cond('safeguard', 'duration'), cond('safeguard', 'durationCallback')],
  IMPL('turn/status_moves.rs::run_status_move', 'the safeguard arm: never-miss so NO accuracy roll, and the FIXED duration 5 (gen-3 durationCallback returns a constant; the Persistent +2 is gen5+) means the cast is entirely DRAW-FREE. Sets SideState::safeguard = 5 and emits |-sidestart|<side>|Safeguard (the BARE name — Light Screen prefixes `move: `, this does not). An already-up re-cast is addSideCondition-false with no onSideRestart, so it takes the [still] did-nothing form plus a bare -fail on the USER, also draw-free. landed=false.'));
add([cond('safeguard', 'onSideResidualOrder'), cond('safeguard', 'onSideEnd')],
  IMPL('turn/residuals.rs::run_residuals', 'ResidualAction::SafeguardDuration at order 4 / SIDE_CONDITION_SUBORDER / speed 0 — the reflect(1)/lightscreen(2)/mist(3) sibling. Decrement, |-sideend|<side>|Safeguard at 0, then the D4 END-branch continue. Only the SAME condition on the OTHER side ties, and that tie is real: BOTH sides warded draws ONE extra random(0,2) per residual (probe-measured 8 -> 9 on a mirror), pinned by both_sides_safeguard_ties_the_side_residual_and_draws_the_shuffle.'));
add([cond('safeguard', 'onSetStatus')],
  IMPL('turn/status.rs::try_set_status_impl', 'the (2b2) block, DRAW-FREE: the gen3ou handler-sort shuffle already fired and safeguard sorts into its OWN group, leaving the 2 Standard clauses a size-2 tie — so the shuffle COUNT is unchanged and the block is SEED-NEUTRAL (the STATUS_IMMUNE-ability pattern). Precedence already-statused > type immunity > STATUS_IMMUNE ability > SAFEGUARD > sleep clause. Emits |-activate|<target>|move: Safeguard for a status-MOVE source only; a blocked SECONDARY is silent and its random(100) still drew. The self-inflicted exemption is unreachable today — run_rest inlines its own status path.'));
add([cond('safeguard', 'onTryAddVolatile')],
  IMPL('turn/secondaries.rs::add_confusion', 'the confusion half of the ward, gated BEFORE the onStart so the random(2,6) is NOT drawn (the Own Tempo shape). Announces only for a PRIMARY (Confuse Ray); a confusion SECONDARY is silent. The yawn arm of the sim handler is moot here — the port blocks a Yawn CAST at its own arm, and a PENDING yawn RESOLVE is exempt in the sim (`if (effect.id === \'yawn\') return`).'));
