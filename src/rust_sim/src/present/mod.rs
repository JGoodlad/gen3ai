...
//! | ~~R1b~~ | **FINDING PE-R1b** — poke-env's toxic count ticks per `\|turn\|`: one ahead for a mon that entered after the residual, one behind between the residual and the next `\|turn\|`; the view reads the STAGE (the residual `[from] psn` chips since its switch-in) | `Pokemon.end_turn` | `PMon::note_residual_chip` |
