...
class RewardTracker:
    ...
    def __init__(self, reward_fn_factory, our_slots: SlotRegistry, opp_slots: SlotRegistry):
        ...
        # (A per-battle ProgressClock lived here until the shaped-reward deletion, 2026-09-26: it
        # existed only so the eval-side reward could charge the no-progress tax. The reward is the
        # terminal alone now, so nothing on this path reads a clock.)
