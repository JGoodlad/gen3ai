...
def test_a_pre_v115_config_is_refused_and_a_current_one_records_OFF():
    """The v115 `setdefault` branch (weight → 0.0) is FLOORED AWAY: gen3_event_record_v2 raised
    MIGRATION_FLOOR to 121 (the branch is archived verbatim in `_migrate_config`'s v97–v120
    history), so a v114 config is a pre-generation checkpoint and is REFUSED with the
    ...
    with pytest.raises(ModelVersionError, match="PRE-GENERATION"):
        _migrate_config({"config_version": 114})
    fresh = _fresh_current_config()
    out = _migrate_config(dict(fresh))
    assert out["config_version"] == MODEL_CONFIG_VERSION >= 121
    assert 'win_prob_strata_weight' in fresh, 'win_prob_strata_weight'
    assert out['win_prob_strata_weight'] == 0.0, 'win_prob_strata_weight'
    ModelVersion(**out)
    # A RECORDED value passes through the current migration untouched (a pre-floor one is refused).
    with pytest.raises(ModelVersionError, match="PRE-GENERATION"):
        _migrate_config({"config_version": 114, **{'win_prob_strata_weight': 0.5}})
    kept = _migrate_config(_fresh_current_config(win_prob_strata_weight=0.5))
    assert kept['win_prob_strata_weight'] == 0.5
    assert getattr(ModelVersion(**kept), 'win_prob_strata_weight') == 0.5
