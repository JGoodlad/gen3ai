...
from agents.training.cf_audit_render import render_markdown  # noqa: F401 (re-export)
from agents.training.cf_audit_twin import (TWIN_CONTRASTS, TWIN_HEADS,  # noqa: F401 (re-export)
                                           attach_twin_heads, paired_head_read, shadow_read,
                                           twin_resolution_read)
from agents.training.stats import (MIN_CELL_N, MIN_SUBCELL_N,  # noqa: F401 (re-export)
                                   cluster_bootstrap_ci, cluster_bootstrap_diff_ci,
                                   sd_true_excess, spearman, wilson_ci)
...
# ---------------------------------------------------------------------------
# Statistics — the estimators themselves live in `agents.training.stats`
# ---------------------------------------------------------------------------
#
# They were lifted there (2026-09-06) because none of them knows what a label, a battle or a
# bias map is: they are pure NumPy, and a second consumer should not have to import this
# instrument to get a Wilson interval. They are re-imported at the top of this module, so
# `from agents.training.cf_audit import wilson_ci` — which `cf_producer` and the tests do —
# keeps resolving.
...
# ---------------------------------------------------------------------------
# The TWIN-HEAD paired read, the SHADOW critic, and the markdown renderer
# ---------------------------------------------------------------------------
#
# The twin-head section moved to `agents/training/cf_audit_twin.py` and `render_markdown` to
# `agents/training/cf_audit_render.py` (2026-09-06, the file-size ratchet's second cut of the
# 1,000-2,000 band). Neither knows anything this module does not already give it — the twin read
# takes labelled rows, the renderer takes a finished bias map. Both are re-imported at the top of
# this module, so `from agents.training.cf_audit import render_markdown` — which the tests and
# the golden do — keeps resolving.
