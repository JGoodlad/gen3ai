...
- **Any change to the phase structure or forward math is a structural change → bump `ARCH_SIGNATURE`** in `model_version.py` (current: `gen3_own_spread_v1`). Pure decompositions still change state_dict keys, so old checkpoints must fail loudly. Re-sourcing or re-meaning an obs block (e.g. v9, where own IV/EV/nature went from constant fallbacks to real values via the poke-env `backfill_teambuilder_spread` fix) is likewise retrain-class even when the dim is unchanged.
