...
extractor and will break — doubly so since v51 (`gen3_pointer_native_v1`): the policy's `_build`
deletes the flat `action_net` and the action logits come from the `PointerNativeActionHead` over
the extractor's `last_pointer_inputs` stash (see the v51 versioning note below). The startup `_run_roundtrip_test` and the snapshot/feature tests all
...
real training run — `refine_proj` (v31/v33), `outgoing_proj` (v36), `status_in_proj`/`status_out_proj`
(v37), `film_pi`/`film_vf` (v44), plus the belief heads (`MoveBelief.move_head`, `SpreadBelief.*`,
...
mon↔mon block. Each family's map is a ZERO-INIT
