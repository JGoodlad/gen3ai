"""Public-value head — module build, byte-identical-off, gradient gating, and the v43 version gate
(gen3_pubval_aux_v1; mirrors win_prob_head_test, the pattern this head reuses)."""
...
def _build(ek, space, mode):
    return Gen3FeaturesExtractor(space, **{**ek, "pubval_mode": mode})
...
def test_off_is_byte_identical_dims(ek_and_space):
    ...
    assert f_none.pubval_head is None and f_none.last_pubval_logits is None
    assert f_ro.pubval_head is not None and f_sh.pubval_head is not None
...
def test_stash_shape_and_independent_of_win_head(ek_and_space):
    """The pubval head stashes a [B,1] logit; the win head stays None (independent modules)."""
    ek, space, total = ek_and_space
    f = _build(ek, space, "read_only")
    f({"observation": torch.zeros(4, total)})
    assert f.last_pubval_logits.shape == (4, 1)
    assert f.win_head is None and f.last_win_prob_logits is None


def _trunk_grad_present(fe, total):
    fe.zero_grad()
    fe.forward_internal({"observation": torch.zeros(1, total)})
    fe.last_pubval_logits.sum().backward()
...
def test_version_records_mode():
    assert _ver("shaping").pubval_mode == "shaping"
    assert _ver().pubval_mode == "none" and _ver().pubval_coef == 0.0
...
def test_mode_mismatch_fatals(saved, current):
    with pytest.raises(ModelVersionError, match="pubval_mode"):
...
def test_coef_not_version_locked():
    """pubval_coef is training-only — a mismatch must NOT block a load (it touches no forward)."""
    a, b = _ver("shaping"), _ver("shaping")
    a.pubval_coef, b.pubval_coef = 0.05, 0.5
