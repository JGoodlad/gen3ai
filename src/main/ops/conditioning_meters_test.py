...
def test_the_provisional_contrast_row_declares_why_it_can_have_no_floor() -> None:
    ...
    # the contrast row and the v6 OPTIMAL-reference family are the ONLY rows without a verdict,
    # and for two different reasons: the contrast has no floor the controls could supply, the
    # optimal rows are descriptive ceilings rather than treatment effects.
...
# ------------------------------------------------- v6: the LATE WINDOWS and the OPTIMAL reference
#
# The N-curve (`measurements/winprob_refit_ncurve_2026-09-10/` §3) established that on a
# matched-team frame the opponent is UNOBSERVABLE at turn 1 — Gen 3 has no team preview, so a
# turn-1 spread ratio of 0 is Bayes-optimal rather than a defect — and becomes observable by turns
# 4-10. The rows below are what a read on that window needs, plus the reference level that says
# how much of the attainable spread `V` is actually emitting.
#
# 🚨 THE FIRST TEST IS THE ONE THAT MATTERS: the v5 rows must be BIT-IDENTICAL. Every landed
# ladder read was written from them.

_GOLDEN = os.path.join(os.path.dirname(__file__), "conditioning_meters_v5_golden.json")


def _golden_case(tmp_path, case: str):
    if case == "plant_separating":
        _plant(tmp_path / case, wr_by_opp=_WRS, v_of=_separating, seed=11)
        return CM.conditioning_block(str(tmp_path / case), 100, boot=150, ladder="off", seed=11)
    _plant_teams(tmp_path / case, v_of=_team_and_board, seed=12)
    return CM.conditioning_block(str(tmp_path / case), 100, boot=150, ladder="off", seed=12)


@pytest.mark.parametrize("case", ("plant_separating", "plant_teams"))
def test_every_pre_v6_row_reproduces_BIT_IDENTICALLY(tmp_path, case) -> None:
    """🚨 The v6 rows are added BESIDE the registered ones and may not perturb a single one.

    Three landed critic-ladder reads and the ladder's floor json were written from the v5 rows; a
    row that moved by a float ulp because a bucket was added to `rollup` or a meter to the
    bootstrap's key list would silently re-price every one of them. The golden was generated by
    the PRE-change tree (`git show HEAD:src/main/ops/conditioning_meters.py`, run from the main
    checkout) and is compared as an EXACT float equality — not `approx` — because the claim is
    bit-identity and an `approx` would pass on the very drift this guards.

    The CI bounds are compared too: a new meter appended to the bootstrap's `draws` dict, or a new
    `cell_stats` call inside the loop, must consume NO randomness from the shared stream. If it
    did, every pre-existing interval would shift while every point stayed put — the failure this
    half catches and the points cannot.
    """
    golden = json.loads(open(_GOLDEN).read())[case]
    blk = _golden_case(tmp_path, case)
    assert set(golden["points"]) <= set(blk["points"]), (
        "a v5 row DISAPPEARED: " + str(sorted(set(golden["points"]) - set(blk["points"]))))
    for key, want in golden["points"].items():
        assert blk["points"][key] == want, f"{key}: {blk['points'][key]!r} != {want!r}"
    for key, (lo, hi) in golden["ci"].items():
        got = np.percentile(blk["_draws"][key], [2.5, 97.5])
        assert float(got[0]) == lo and float(got[1]) == hi, f"{key}: {got} != {[lo, hi]}"


def test_the_late_window_rows_exist_and_carry_their_own_intervals(tmp_path) -> None:
    _plant(tmp_path, wr_by_opp=_WRS, v_of=_separating, seed=21)
    blk = CM.conditioning_block(str(tmp_path), 100, boot=200, ladder="off", seed=21)
    for key in (CM.SPREAD_RATIO_T4_10, CM.SPREAD_RATIO_T11_24,
                CM.OPP_CLASS_AUC_T1_3, CM.OPP_CLASS_AUC_T4_10):
        assert key in blk["points"], key
        assert blk["_draws"][key].size > 100, key


def test_a_separating_head_reads_near_one_in_every_window_and_a_marginal_head_near_zero(
        tmp_path) -> None:
    """The windows are new; the arithmetic inside them is the registered one, so the two planted
    regimes must separate in `t4_10` and `t11_24` exactly as they do in `t1_3`."""
    _plant(tmp_path / "sep", wr_by_opp=_WRS, v_of=_separating, seed=22)
    _plant(tmp_path / "mar", wr_by_opp=_WRS, v_of=_marginal, seed=22)
    sep = CM.conditioning_block(str(tmp_path / "sep"), 100, boot=200, ladder="off", seed=22)
    mar = CM.conditioning_block(str(tmp_path / "mar"), 100, boot=200, ladder="off", seed=22)
    for key in (CM.SPREAD_RATIO_T4_10, CM.SPREAD_RATIO_T11_24):
        assert sep["points"][key] > 0.8, key
        assert mar["points"][key] < 0.15, key
    for key in (CM.OPP_CLASS_AUC_T1_3, CM.OPP_CLASS_AUC_T4_10):
        assert sep["points"][key] > 0.90, key
        assert abs(mar["points"][key] - 0.5) < 0.20, key


def test_the_late_window_buckets_hold_exactly_the_turns_they_name() -> None:
    arr = np.zeros(6, dtype=CM.STATE_DTYPE)
    arr["battle"] = "b0"
    arr["turn"] = [1, 3, 4, 10, 11, 24]
    arr["V"] = 0.5
    arr["w"] = 1.0
    b = CM.rollup(arr)
    assert b["n_t1_3"][0] == 2 and b["n_t4_10"][0] == 2 and b["n_t11_24"][0] == 2
    assert b["n_all"][0] == 6


# ---- the OPTIMAL reference

def test_a_separating_heads_spread_IS_mostly_opponent_identity(tmp_path) -> None:
    """A SEPARATING head's `V` is planted as the opponent's own win rate, so almost all of its
    between-opponent spread IS opponent identity and the decodable part sits near `V`'s ratio."""
    _plant(tmp_path, wr_by_opp=_WRS, v_of=_separating, seed=23)
    blk = CM.conditioning_block(str(tmp_path), 100, boot=120, ladder="off", seed=23)
    for window, row in CM.OPT_WINDOWS:
        opt = blk["points"][CM.OPT_RATIO[window]]
        assert 0.6 < opt < 1.4, (window, opt)
        assert abs(opt - blk["points"][row]) < 0.5, (window, opt, blk["points"][row])


def test_a_marginal_heads_decodable_part_is_near_zero_and_so_is_its_spread(tmp_path) -> None:
    """🚨 THE READING THE ROW EXISTS FOR. A head that emits one number regardless of opponent
    carries NO opponent identity, so the decodable part is ~0 — and so is its spread. Without this
    row a reader compares 0.05 to 1.0 and calls it a failure, when the opponent channel available
    at that turn may be empty."""
    _plant(tmp_path, wr_by_opp=_WRS, v_of=_marginal, seed=24)
    blk = CM.conditioning_block(str(tmp_path), 100, boot=120, ladder="off", seed=24)
    for window, row in CM.OPT_WINDOWS:
        assert blk["points"][CM.OPT_RATIO[window]] < 0.35, window
        assert blk["points"][row] < 0.35, window


def test_the_two_regimes_decodable_parts_are_ordered_by_how_much_V_carries(tmp_path) -> None:
    _plant(tmp_path / "sep", wr_by_opp=_WRS, v_of=_separating, seed=25)
    _plant(tmp_path / "mar", wr_by_opp=_WRS, v_of=_marginal, seed=25)
    sep = CM.conditioning_block(str(tmp_path / "sep"), 100, boot=120, ladder="off", seed=25)
    mar = CM.conditioning_block(str(tmp_path / "mar"), 100, boot=120, ladder="off", seed=25)
    for window, _row in CM.OPT_WINDOWS:
        assert (sep["points"][CM.OPT_RATIO[window]]
                - mar["points"][CM.OPT_RATIO[window]]) > 0.4, window


def test_the_posterior_is_fitted_out_of_fold_and_never_scores_its_own_battle() -> None:
    """The decodable part would be an overstatement if the posterior saw the battle it scores: the
    bin a state falls in would carry that state's own opponent. One battle held out has to take
    the OTHER battles' bin means."""
    rng = np.random.default_rng(3)
    groups = np.repeat([f"b{i}" for i in range(40)], 5)
    v = rng.random(200)
    z = np.where(v > 0.5, 0.8, 0.2)
    w = np.ones(200)
    out = CM.oof_opt_value(v, z, w, groups, seed=1)
    assert np.isfinite(out).all()
    # a purely separable relation is recovered out of fold...
    assert np.corrcoef(out, z)[0, 1] > 0.9
    # ...but never by memorising: a constant target cannot be beaten, and a target that is pure
    # noise given V decodes to the grand mean, not to itself.
    noise = rng.random(200)
    flat = CM.oof_opt_value(v, noise, w, groups, seed=1)
    assert abs(float(np.mean(flat)) - float(np.mean(noise))) < 0.06
    assert np.corrcoef(flat, noise)[0, 1] < 0.35


def test_the_optimal_rows_are_PROVISIONAL_and_can_never_be_DETECTED() -> None:
    """The decomposition term is DESCRIPTIVE. It is not a treatment effect, it is NOT a bound on
    the row beside it, and it must never carry a verdict. The declaration says both."""
    for window in CM.OPT_RATIO.values():
        m = CM.METER_BY_KEY[window]
        assert m.provisional, window
        assert window in CM.PROVISIONAL_KEYS
        assert "pass/fail" in m.provisional_why
        # 🚨 the declaration must carry the refutation, not just the caveat: the first real read
        # had V ABOVE this row at every window, and a future reader who takes it for a ceiling
        # will misread every arm.
        assert "NOT AN UPPER BOUND" in m.provisional_why


def test_the_frame_carries_both_ratios_so_the_part_is_never_quoted_alone(tmp_path) -> None:
    _plant(tmp_path, wr_by_opp=_WRS, v_of=_separating, seed=26)
    blk = CM.conditioning_block(str(tmp_path), 100, boot=60, ladder="off", seed=26)
    ref = blk["frame"]["optimal_reference"]
    for window, row in CM.OPT_WINDOWS:
        assert ref[window]["ratio_V"] == pytest.approx(blk["points"][row])
        assert ref[window]["ratio_optimal"] == pytest.approx(blk["points"][CM.OPT_RATIO[window]])
