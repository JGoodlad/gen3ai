...
Each TurnDelta slot (155 dims, layout in `src/agents/observation/turn_delta_encoder.py`; all offsets computed from named `OFFSET_*` / `*_DIM` constants — never hardcode indices). TurnDelta is now **folded from the event log** (`Gen3Battle.events_since(cursor)` per decision window) rather than diff-heuristics.
