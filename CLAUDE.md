...
| `designs/ops/TRAINING_RUN_SOP.md` | **How a training run is OPERATED** — the two independent pre-launch checks, the four watch layers and the 55-minute fallback cron, kill/relaunch, the read; and where sessions find the ORCHESTRATOR (`~/.claude/projects/-home-goodlad-dev-gen3ai/ORCHESTRATOR`). `designs/ops/` is the standard home for SOPs; `designs/ops/ORCHESTRATOR_SOP.md` is the orchestrator session's (OPUS subagents only) |
