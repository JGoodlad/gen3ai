...
### ai_v10 (OPEN — the exploiter-SCALING chapter, + four forward docs)
...
*what transfers between teams*. The forward docs: the flywheel, the outcome latent,
...
2026-08-21 probe triad, gated on bias meters; the C6-compliant new mechanism), and
[`design_advantage_gated_distillation.md`](ai_v10/design_advantage_gated_distillation.md)
(2026-08-25 — the DEEP-BRANCH fix for the fold, after five +3M arms eliminated ecology, coefficient,
state-gate and teacher-count and left `pi_features` **binary at 21.87 without any KL / 12.5–13.6
with any**: a two-axis ladder separating the distillation TARGET FORM from its JUDGE, plus a
`rank/policy_pr` tripwire calibrated so every known-bad arm trips and the known-good control does
not. It formally contests the flywheel's **D-F** "always full-distribution" decision).
...
distribution teaches, **ai_v12 is what the win-probability head becomes when it stops being a
barometer and becomes the value function**.

Two designs, a runbook and a capstone:
...
- [`design_winprob_behavior_coupling.md`](ai_v12/design_winprob_behavior_coupling.md) — the plan of
  ...
  `--win-prob-pbrs-coef` (PBRS, `γφ(s′) − φ(s)`, φ detached — config v104), routes 2+3
  `--search-teacher-mode winprob_oneply`. **Probe L is what fires the chapter:** the head ranks an
  alternative above the played action on **96.4% of immune whiffs** (+0.213 over the tightest
  control, dice-invariant) while the policy samples that alternative at a median **p = 0.002** — the
  head knows and the policy does not act on it. The distillation branch fires, so route 2 is named
  the first arm and route 1's E1 ladder was re-sized by two orders of magnitude in the same pass.
- [`design_winprob_only_critic.md`](ai_v12/design_winprob_only_critic.md) — **the design of record**
  (2026-09-06), implemented the same day as `gen3_winprob_critic_mode_v1` / config **v109**. Its §6
  gap list is worked in its own stated order (A2 census → B2/B3 → the mode) and its §5.3 is the
  DELETION LIST for the later default flip: `--value-dist-*` + `ValueDistHead` + `check_value_dist`,
  `--value-from-dist` + `--allow-value-from-dist-change`, `--win-prob-pbrs-coef` /
  `--win-prob-pbrs-source` + `agents/training/winprob_pbrs.py`, `--win-prob-coef` (one critic, one
  coefficient), `--value-tail-weight`, `--score auto`. **`use_popart` is KEPT and set false** — the
  two-currency machinery is correct and wanted again the moment the objective stops being
  terminal-only — and `--draw-penalty` becomes a REFUSAL rather than a deletion. `cf_twin_heads`,
  `cf_shadow_critic`, `cf_evidential`, `q_winprob_mode` and `--td-aux-coef` are all KEPT and get
  *more* useful once `win_head` is the critic. The doc's own §5.2 says `MIGRATION_FLOOR` **108**;
  the mode landed at v109, so the flip's floor is **110**.
- [`launch_runbook.md`](ai_v12/launch_runbook.md) — three generation-scale arms, **SPARSE / SELF-φ /
  FROZEN-φ**, identical but for where the potential comes from, all at `{win +1, loss −1, draw −1}`;
  `SELF − SPARSE` prices self-shaping, `FROZEN − SELF` prices maturity plus exact-vs-approximate
  invariance, `FROZEN − SPARSE` the total worth of outcome-grounded shaping. A paired **5M pre-test**
  runs ahead of all three and sizes them in GPU-hours rather than generations. Every argv in it
  passes `main.checkargs` at exit 0 and has run as a `--debug --steps 8000` CPU smoke, and
  `src/main/launch_runbook_test.py` parses the blocks **out of the document** through the live
  parser — so a flag deleted anywhere in the tree fails a test naming this doc rather than a launch
  two days later. It is a runbook, not a registration: the arms were registered in the ledger
  (`e22bd08`, `627ab58`, `cfbc9bf`, `2d38a4a`, `db9bb5c`, `4d22ae4`, `132d198`).
- [`probe_risk_modulation_capstone.md`](ai_v12/probe_risk_modulation_capstone.md) — owner-ordered
  2026-08-30. Does a P(win) value function buy correct risk modulation? Three offline instruments
  (accuracy-tradeoff curve · Explosion timing · CRN-reroll spread curve) plus a guess-point entropy
  companion, with **frozen per-arm predictions**: sparse steepest slope, shaped-world shallowest. A
  FLAT sparse slope falsifies "P(win) buys risk for free" **and must be reported as loudly as a
  pass**.

Also in the folder: the 40-team slate (`team_slate_40.{json,md}` + `team_slate_build.py`) and
`promotion_exclusions.json`, the exclusion list `python -m main.promote_teams
...
**The era boundary is censused, not decided.**
[`research_state/era_boundary_deprecation_2026-09-06.md`](research_state/era_boundary_deprecation_2026-09-06.md)
prices the v110 flip the way v51 was priced — floor raise, migration collapse, purge v3 — with the
list of runs that stop loading and the era-checkout recipe that keeps them readable.
