...
## v115 — `--win-prob-strata-weight`: the win-prob BCE's opponent MIX (`gen3_winprob_strata_weight_v1`)

**2026-09-09. Arm 7 of the critic ladder. ONE training-only loss weight; default `0.0` = OFF and
the loss is BIT-identical.**

### Why

[`designs/research_state/measurements/winprob_head_refit_2026-09-09/`](research_state/measurements/winprob_head_refit_2026-09-09/README.md)
refit the win head alone on a FROZEN `value_pooled`, out of fold under battle-grouped CV and
HT-reweighted, on `ai_v12_02_winprob_critic` @74M and the ladder control `ai_v12_11_ladder_ctrl10M`
@10M. Against the **terminal 0/1 target the online head actually trains on** it reproduced the
online failure exactly — turn-1–3 between-opponent spread ratio 0.149 → 0.000 (Δ [−0.055, +0.234])
and 0.066 → 0.068 (Δ [−0.044, +0.042]), turn-1 opponent-class decode 0.532 → 0.546 against
`value_pooled`'s own 0.846. Against the same label with its per-episode variance removed the SAME
head on the SAME features recovered a DETECTED part (0.149 → 0.323, 0.066 → 0.259). A head
initialised FROM the online weights lands where a scratch head lands under both targets, so the
online head is **not in a basin** and the head-side-optimisation treatment class is RULED OUT.

The mechanism is arithmetic: only **10.2 % / 14.4 %** of the terminal label's variance lies BETWEEN
(cycle, opponent) cells, so a head minimising a proper scoring rule buys its resolution from the
board and its own team — which is cheaper — and the opponent component is ~10 % of the objective at
*any* sample size. §11 names opponent-stratified weighting of the win-prob loss as the highest
expected-effect-to-cost lever on the list: it raises the between-cell share **directly**, with no
new labels, no new machinery and no extra rollout cost.

### What it is

Each state's BCE term is multiplied by its opponent CLASS's weight, computed ONCE per rollout over
the whole buffer's KNOWN rows and held constant for every epoch and minibatch:

```
raw_c = min(f_c ** (-s), 8.0)                    s = --win-prob-strata-weight in [0, 1]
w_c   = raw_c / Z,   Z = (Σ_c n_c · raw_c) / N   ⇒ mean(w) over the known rows == 1 exactly
```

`s = 0` returns no vector at all, so the caller takes the original masked mean unchanged (bit-
identical, not approximately equal). At `s = 1` and no clipping each class contributes `N / C` to
the objective; `s` interpolates the exponent monotonically. The mean-weight-1 normalisation is what
keeps `s` a re-pricing of the MIX rather than a rescaling of the value gradient — without it an arm
could not tell "balanced the classes" from "raised the critic's step size". The denominator stays
`N` and not `Σ mask·w`: dividing by the weighted count would renormalise per minibatch and undo the
buffer-level balance the weights were computed to produce.

**Vocabulary: the four `opp_class` codes** — `bot` / `pool` / `stable` / `exploiter`. One class per
BOT NAME plus a single `selfplay` class was considered and is **not available**: the archetype and
the pool snapshot's step are drawn per EPISODE inside `MaskableAgentWrapper._select_episode_opponent`
and never reach the observation (`value_sidecar.py` declined it for the same reason). So the lever
balances the between-CLASS share and leaves within-bot heterogeneity in episode proportion — stated
because it bounds what the arm can move.

🚨 **THE 8× CAP BINDS AT THE PRODUCTION MIX, DELIBERATELY.** At the measured ~10 % bot / ~90 %
self-play post-promotion mix, `s = 1` asks for 10× on the bot stratum and gets 8×, so the objective
splits **44/56 rather than 50/50** — a 4.4× re-pricing of the between-class signal with a hard bound
on the per-row gradient weight, instead of 5× with an unbounded one. Pinned by test, and tabulated in
`designs/training/critic_and_value_losses.md`, so a reader comparing `strata_share_*` against parity
finds the reason rather than a discrepancy.

**`--critic winprob` is REQUIRED** (`combination_checks.winprob_strata_needs_the_winprob_critic`),
never a silent no-op: under `shaped` that BCE is an auxiliary readout and the critic is the scalar
value net, so a stratified weight there would re-price a DIAGNOSTIC and leave the value function
untouched. The counterfactual and twin-head callers of `_win_prob_loss` stay unweighted and a test
pins it — they score foreign recorded states whose opponent mix belongs to the label factory.

### The read

`win_prob/strata_*`, once per rollout: `strata_w_<class>` (the weight vector itself),
`strata_frac_<class>` (the before), `strata_share_<class>` (the after), `strata_w_entropy`
(normalised, 1.0 = balanced — the one number that says the lever landed), `strata_w_min/max`,
`strata_capped`, `strata_n_classes`, `strata_rows`; per minibatch `loss` vs `loss_unweighted` and
`strata_row_w_mean`. A flat or absent family means the lever never fired.

### Version + gates

`win_prob_strata_weight` is a **v115 `ModelVersion` field of the `td_aux_coef` class** — recorded
for provenance and flagless-resume read-back, never compared by `check_compatible` (it reweights a
loss and touches no forward pass or weight shape, and gating a frozen eval/pool/distill opponent on
it would be a false rejection). A pre-v115 config migrates to `0.0`, a RECORD and not a guess: the
field did not exist. **No `ARCH_SIGNATURE` bump, no `MIGRATION_FLOOR` change, and not a
`flag_registry.py` row** — that registry declares EXTRACTOR toggles and this builds no module. It IS
declared in `arch_tables._COEF_MODULE` (→ `win_head`) so a production config that ever adopts it
cannot be silently dropped from the generated table the way `intent_label_bot_weight` was from v97.

`agents/training/winprob_strata_weight_test.py` (37: OFF returning no vector and the loss being
bit-identical; a half-present pair taking the unweighted path; exact per-class parity at `s = 1`;
monotone interpolation; a single present class returning None; unlabeled rows excluded from the
frequencies; mean-weight-1 across four mixes × three exponents; the live minibatch read-back; the
cap binding, being reported, and the 44/56 production arithmetic; no class exceeding the cap ratio;
the refusal firing under `shaped` and staying silent under `winprob`, when off, and on an unset
namespace; the range check; argparse defaulting to None so `_resolve` is reachable; the `_resolve`
line; the dataclass field and its default; the pre-v115 migration; the writer, the snapshot rebuild
and BOTH `ModelVersion` construction sites; absence from `check_compatible`; the PPO hparam default;
...
## v116 — `--win-prob-lambda`: λ-RETURN targets for the win-prob BCE (`gen3_winprob_lambda_v1`)

**2026-09-09, the critic ladder's arm 8.** v115 re-priced *which states* the win-prob value loss is
bought from. This one changes *what those states regress toward*, and it comes out of the same
measurement.

**THE FINDING.** Under `--critic winprob` the value loss is a BCE against **one terminal bit copied
to every state of the episode** (`WinProbLabelCallback`). The head refit
(`designs/research_state/measurements/winprob_head_refit_2026-09-09/`) proved the win-prob critic's
conditional miscalibration is a **TARGET** defect and not a head defect: only **10.2 % / 14.4 %** of
that label's variance lies BETWEEN (cycle, opponent) cells, so an early-stopped on-policy learner
minimising a proper scoring rule shrinks the weak axes toward the marginal. The shape of that is the
turn-1 read — the critic barely separates opponents or its own team at turn 1 (between-opponent
spread ratio **~0.1**) although its features carry both, while **mid- and late-game values already
separate opponents at ~0.5–0.8 over all states**. The information exists inside the episode and
never reaches turn 1.

**THE FLAG.** `--win-prob-lambda <0..1>`, default **`1.0` = OFF and BIT-identical** — and bit-identical
by SKIPPING the recursion whole rather than by computing an identity, which is also what keeps the
truncation convention unchanged at the default. Below 1.0, per episode inside each rollout buffer:
...
γ = 1 and the clean-world stream is terminal-only (`--no-hand-shaping --terminal-indicator`), so an
n-step return carries no intermediate reward term at all and **IS** `V(s[t+n])`; the λ-weighted
average over n collapses to that single backward pass. A state `d` steps from its terminal keeps
weight **λ^d** on the outcome. The loss is unchanged — the same masked-mean
`binary_cross_entropy_with_logits`, now against a SOFT target in [0,1], which is exactly what a
proper scoring rule generalises to (it scores the target's expectation).

**`V` IS THE RECORDED, PRE-UPDATE VALUE** — `rollout_buffer.values`, which under this critic *is*
`sigmoid(win logit)` (`policy._critic_value`) — and not a re-forward inside `train()`. A target
recomputed from the current weights would move under its own gradient across the ten epochs; the
collection-time values are a fixed point of this rollout by construction.

**THE BUFFER BOUNDARY IS ITS OWN FLAG.** `--win-prob-lambda-truncated {bootstrap,mask}`, default
`bootstrap`. A rollout ends mid-episode in every env column; those states have no outcome and carry
`win_mask = 0` today. Their successor is `s_T`, so the recursion needs no special case —
`G = (1−λ)·V(s_T) + λ·V(s_T) = V(s_T)` — with `V(s_T)` from the same `model._last_obs` forward SB3's
own GAE bootstrap and `winprob_pbrs` use. `bootstrap` gives them that target and **UNMASKS** them;
`mask` leaves them excluded. It is a FLAG rather than a constant so a read can attribute an effect to
*the target change* rather than to *the extra rows*, and `win_prob/lambda_unmasked` counts them per
rollout (measured on the `--debug` smoke: 7–92 rows of 2,048). It is INERT at λ = 1.0.
🚨 An episode that ended with **no recorded outcome** is never unmasked: the trailing segment is
computed from `episode_starts` and `model._last_episode_starts` rather than inferred from
`win_mask == 0`, because an episode that finished without a `win_outcome` in its info is also
unlabelled and its rows would anchor at `y = 0` — a fabricated label.

**WHERE IT RUNS, AND WHY NOT IN `train()`.** In `WinProbLabelCallback._on_rollout_end`, immediately
after the back-fill it replaces. It needs the buffer's `[n_steps, n_envs]` shape — `get()` flattens
and shuffles it — and it needs the post-rollout observation. The metrics are stashed on the model and
folded into `win_prob_metrics` at the top of `train()` so they ride the ordinary `win_prob/` prefix.

**THE TB FAMILY**, once per rollout: `lambda`, `lambda_rows`, `lambda_unmasked`,
`lambda_bootstrap_frac` (share of scored rows whose target is mostly later values, λ^d < 0.5),
`lambda_weight_mean`, `lambda_target_shift` (mean |G − y| on rows that HAVE an outcome),
`lambda_loss` / `lambda_loss_terminal`, `lambda_truncated_bootstrap`, `lambda_bootstrap_fallback`.
🚨 **`lambda_loss` and `lambda_loss_terminal` are scored on the SAME states with the SAME recorded
predictions**, so their difference isolates the target change and cannot be a step of learning — a
post-update per-minibatch pair was rejected because the recursion overwrites `win_target` in place
and `y` does not survive the shuffle. An ABSENT `win_prob/lambda_*` family means λ = 1.0.

**COMPOSITION.** Orthogonal to `--win-prob-strata-weight` (that one weights ROWS, this one re-aims
them; neither reads the other). **Disjoint** from the counterfactual labels: `cf_winprob_term`
re-applies the head to FOREIGN recorded states from `<run>/cf_labels/` with their own tight-MC
labels and never touches `win_target`, so no precedence rule is needed and the two losses are simply
added. ⚠️ The **value sidecar's `target` column follows the flag** — it runs immediately after this
callback, so under λ < 1 it holds the λ-return rather than the outcome. `SIDECAR_SCHEMA` moves to
**2** and the header gains `win_prob_lambda` / `win_prob_lambda_truncated`, so a v1 reader can refuse
rather than quietly average a quantity it believes is a 0/1 outcome.

**RECORDED at config v116**, the `td_aux_coef` class: provenance + flagless-resume read-back, never
compared by `check_compatible` (they re-aim a loss target computed in a post-collection callback and
touch no forward pass or weight shape, and gating a frozen eval/pool/distill opponent on them would
be a false rejection). Pre-v116 migrates to `1.0` / `"bootstrap"` — a RECORD, not a guess: λ = 1.0
**is** the terminal-bit target every prior run trained against, and the truncation mode is inert at
that λ. No `ARCH_SIGNATURE` bump, no `MIGRATION_FLOOR` change, and NOT a `flag_registry.py` row —
but `win_prob_lambda` IS declared in `arch_tables._COEF_MODULE` → `win_head`, so a production config
that ever adopts it cannot be silently dropped from the generated table.

`--critic winprob` is REQUIRED and the refusal lives in `combination_checks`, so both
`resolve_config` and `python -m main.checkargs` report it: under `shaped` the BCE is an auxiliary
readout **and** `rollout_buffer.values` holds a PopArt-normalised shaped return rather than a
probability, so the blend would be a category error fed into a BCE.

GATES: `winprob_lambda_test.py` (35) + a `combination_checks` ARGVS row. Smoke:
`--debug --steps 10000 --critic winprob --win-prob-lambda 0.9` (clean-world composition) trains to
`Training complete`, publishes the whole family (`lambda_bootstrap_frac` 0.77–0.80,
...
## v117 — `--win-prob-dense-aux`: DENSE AUXILIARY targets beside the win-prob BCE (`gen3_dense_aux_v1`)

**2026-09-10, the critic ladder's arm 9.** v115 re-priced *which states* the win-prob value loss is
bought from and v116 changed *what those states regress toward*. Both acted on the same one bit.
This one stops acting on it and **adds 25 targets beside it**.

**THE FINDING IT IS BUILT AGAINST.** The win-prob head trains on one bit per episode copied to every
state, and only **~10 %** of that label's variance lies BETWEEN opponents
(`designs/research_state/measurements/winprob_head_refit_2026-09-09/` §6) — so the head shrinks the
weak axes (opponent, own team) toward the marginal although its features carry them. **Four 10M
levers moved nothing at ±0.01 on bot resolution** (ledger *THE ARMS AT 400 GAMES*), and every one of
them re-weighted, re-aimed or re-priced the SAME bit. The literature's answer to a one-bit terminal
signal is KataGo's (Wu 2019 §3): keep the win target and add **dense auxiliary targets that share
the win's cause** — ownership of every point of the board, and the final score — reported there as a
large gain in learning efficiency. Our analogue of "ownership of every point" is **per-Pokémon
end-of-battle outcomes**: 25 numbers per state instead of one bit, 24 of them facts about a NAMED
ENTITY the state's own observation also carries, so the gradient they deliver runs along exactly the
per-entity axes a pooled bit cannot separate.

**THE FLAG.** `--win-prob-dense-aux <coef>`, default **`0.0` = OFF and BIT-identical** — and
bit-identical by not BUILDING the head at all, so there is no module, no obs key, no callback and no
term. `--critic winprob` is REQUIRED (a `combination_checks` refusal), and `--win-prob-mode != none`
is a `flag_registry` `requires` the extractor constructor enforces.

**THE HEAD.** `DenseAuxHead` (`src/agents/model/dense_aux_head.py`):
`Linear(D_MODEL, 64) → ReLU → Linear(64, 25)` over the same `value_pooled` the win head reads,
zero-init output (so every logit is 0 ⇒ p = 0.5 everywhere, the honest state of knowledge), built
LAST and **not called by the forward at all** — the `CfEvidentialHead` contract, which makes pi/vf
bit-identical at an ARBITRARY weight in it and costs a rollout, an eval and the prober nothing.

    outputs  0..11   SURVIVAL of slot k (our 6 then theirs 6, the OBSERVATION's own team order)
    outputs 12..23   slot k's FINAL HP FRACTION at termination
    output     24    TURNS LEFT, log1p(terminal_turn − this_turn) / log1p(250)

`aux_loss = coef × mean(the three masked-mean BCE terms present)` — a mean of TERMS, not a pooled
mean over 25 columns, so twelve survival outputs cannot outvote the one turns output, and a
...
A sigmoid keeps a bounded-in-[0,1] target in range by construction (no clamping, and no
MSE-through-a-saturating-sigmoid vanishing gradient at 0 and 1, which is where the HP mass sits);
BCE with a soft target is a proper scoring rule for a [0,1] mean, the same family as the win head's
own loss; and one family puts all three terms in the same nats scale, so the single coefficient
means one thing across them. The price is a non-zero entropy floor on the two soft terms, which is
why `aux_hp_mae` / `aux_turns_mae` are published beside them as the interpretable reads.

**THE PER-SIDE KO COUNTS ARE DERIVED, NEVER PREDICTED.** `6 − Σ survived` per side is a linear
function of the survival block, and it is the one target that could not honour the mask (a count is
a sum over slots some of which are unscored). Published as `win_prob/aux_ko_mae_*`.

**TWO MASKS, ANDed with the episode-known bit.** A slot is scored only where it HAS an end-of-battle
fact and where it names an entity THIS state's observation carries. An opponent mon never revealed
has no observed fainted flag and no observed HP: MASKED, never fabricated as "alive at full HP" — a
label that would be wrong in a DIRECTION, since the un-revealed mons of a team we beat are
disproportionately the ones it never got to send out. And opponent slot order IS reveal order, so
the final order is a prefix-stable extension of every earlier state's; scoring a not-yet-revealed
slot would anchor a label to a feature block encoding nothing, which is this arm's own defect one
level down. The env emits the per-state visibility as a REAL value in the `aux_mask` placeholder
and `DenseAuxLabelCallback` ANDs it with terminal availability.

**THE INPUT IS NOT DETACHED, AND THAT IS THE ARM.** Every other head absent from the forward
(`CfEvidentialHead`, the twins, the shadow critic) takes an unconditionally detached input and reads
`grad/*_share` 0.0 by construction. This one takes the live stashed `value_pooled`, so its gradient
reaches the shared trunk exactly as the win-prob loss does under `shaping` (which `--critic winprob`
implies). `grad/dense_aux_share` is the verification and must NOT read 0. It is folded as an `aux`
term at the flag's own coefficient, never at `vf_coef`: there is one critic and these are not it.

**λ DOES NOT REACH THESE TARGETS.** They are terminal FACTS, not returns, and there is no recorded
per-state estimate of "slot 4's final HP" for a recursion to blend. The precedence is STRUCTURAL —
the λ recursion overwrites `win_target`/`win_mask` in place and names no `aux_*` key — and a source
scan pins it. Under λ < 1 the two coexist: the win term regresses toward a soft λ-return, the aux
terms toward the battle's own ending.

**PLUMBING.** Three TRAINING-ONLY obs keys (`aux_target` [25] placeholder, `aux_mask` [25] carrying
the per-state visibility, `aux_turn` [1] the state's turn) on the `win_target` precedent — none is
an `extra_obs_keys` row, because the extractor never reads them. `MaskableAgentWrapper` publishes
the terminal facts from `battle1` at the done step, the same seam `info["win_outcome"]` uses;
`DenseAuxLabelCallback` back-fills them, and the async collector records them inline at its own row.

**RECORDED at config v117 as TWO fields, because they are gated differently.** `dense_aux` (bool) is
STRUCTURAL — the `value_true_team` mould, gated by a bool compare in `check_compatible` because the
head's params are the whole state_dict delta and its only output is a training-side loss, so no
shape error anywhere would catch a flipped flag. `win_prob_dense_aux` (float) is the `td_aux_coef`
class — provenance + flagless-resume read-back, never compared. So a resume may RE-DOSE the arm
freely and may not add or remove its parameters. A pre-v117 config migrates to `False` / `0.0`,
which is not a guess: neither field existed. **No ARCH_SIGNATURE bump, no MIGRATION_FLOOR change**;
`dense_aux` IS a `flag_registry.py` row (`derived=True` off the coefficient, `family=CRITIC`,
`requires=("win_prob_mode",)`), and `win_prob_dense_aux` IS declared in `arch_tables._COEF_MODULE`
(→ `dense_aux_head`). `family=CRITIC` keeps it off the ARCH surface, so
`--arch production --win-prob-dense-aux 1.0` is the documented launch.

GATES: `dense_aux_head_test.py` (12) + `dense_aux_test.py` (24) + a `combination_checks` ARGVS row.
Smoke: `--debug --steps 10000 --critic winprob --win-prob-dense-aux 1.0` trains to `Training
complete`, publishes the whole `win_prob/aux_*` family (`aux_coverage` 0.94–1.00, `aux_masked_frac`
0.30, `aux_auc_own` 0.62 → 0.83 against `aux_auc_opp` 0.54–0.61, `aux_hp_mae` 0.48 → 0.39,
...
## v119 — `--win-prob-rollout-weight`: the ANCHOR loss weight (`gen3_winprob_rollout_weight_v1`)

2026-09-10. Arm 10's **arithmetic half**. `--win-prob-rollout-target` (v118) buys measured `wins/R`
targets for a subsample of the buffer, but the fraction that costs 1× the run's own simulation
budget is `1/(R × 104) ≈ 0.0012`, so at R = 8 the anchored rows are **~0.12 % of the win-prob BCE's
mass**. A treatment carrying 0.12 % of an objective **cannot move the head by arithmetic**, whatever
its labels say — a null at that dose is a fact about the dose. Raising the fraction is unaffordable
(cost is exactly linear in it). This flag is the only lever that raises the treatment's share of the
objective **at fixed simulation cost**: it multiplies the ANCHORED rows' per-row BCE by `k` and
renormalises the vector to mean 1 over the SCORED rows, so the loss SCALE does not move.

    Z = 1 + f·(k − 1);  w_anchor = k/Z, w_other = 1/Z;  anchored share = f·k/Z

At `f = 0.0012`: `k = 32` → 3.7 %, **`k = 64` → 7.1 %** (the registered operating point; the arm's
dose bar is 5–15 % of the mass once λ propagation is counted).

**Default `1.0` = OFF and BIT-identical** — the obs key is not even declared, so the loss takes its
unweighted expression unchanged. **Requires `--win-prob-rollout-target > 0`** (refused in
`combination_checks`: with no anchors the weight vector is a vector of ones and the run is the
unflagged one under a flagged name). Below 1.0 is refused rather than clamped — it is a
coherent-looking instruction for the opposite of this lever.

**COMPOSITION.** With `--win-prob-strata-weight` the two **MULTIPLY** (an overwrite would silently
disable whichever was applied second); each is mean-1 over the buffer and the product is mean-1 up
to a selection/opponent covariance the uniform sampler makes ~0, published as `win_prob/row_w_mean`
rather than assumed. 🚨 Under `--win-prob-lambda < 1` **only the ANCHORS are weighted**, never the
rows that bootstrap toward them: those targets are a MIXTURE of the anchor, the network's own later
values and the copied bit, so weighting them would dose arm 8's channel under arm 10's flag and
would make the delivered dose a function of the episode-length distribution. The λ^k reach is
MEASURED instead — `lambda_return_targets` fills an anchor-SHARE array in its own backward pass (an
out-parameter, so the recursion stays the one place that defines "ends").

**THE CARRIER.** A per-row weight must survive `RolloutBuffer.get()`'s shuffle aligned to its row, so
it rides a third LABEL obs key, **`win_row_w`**, declared by `Gen3Env` only when the flag is above
1.0 and emitted as a placeholder of **1.0, not 0.0** — it is a MULTIPLIER, and a zero placeholder
reaching the loss would zero the whole win-prob term and read as a dead head.
`_apply_rollout_weight` runs AFTER both target treatments, because λ may re-mask rows and the mean-1
normaliser must be computed over exactly the rows the BCE will score.

**NEW TB:** `win_prob/rollout_weight` (the ask), **`rollout_mass_weighted`** (the anchors' share of
the WEIGHTED mass — the DELIVERED dose a read quotes), **`rollout_influence_lambda`** (anchors plus
every row whose λ-target received an anchor contribution, each counted by its λ^k share), and
`win_prob/row_w_mean` (the realised composed row weight).

**RECORDED at config v119**, the `td_aux_coef` class: provenance + flagless-resume read-back, never
compared by `check_compatible`. Pre-v119 migrates to `1.0` — a RECORD, not a guess: no run could
weigh a row it had no flag to weigh with. The read-back matters as much as the fraction's: a restart
that dropped the WEIGHT would keep paying for every continuation while delivering ~1/50th of the
registered dose. No `ARCH_SIGNATURE` bump, no `MIGRATION_FLOOR` change, NOT a `flag_registry.py` row
— but `win_prob_rollout_weight` IS declared in `arch_tables._COEF_MODULE` → `win_head`.

🚨 **The value SIDECAR is deliberately unchanged — no schema bump, not in `QUANTITY_FIELDS`.** That
tuple's rule is "does it change what the `target` COLUMN HOLDS?", not "is it a new flag". A per-row
LOSS WEIGHT changes no row's target, so a v3 file at weight 64 holds the same per-row quantity as
one at 1.0; adding it would refuse to pool two genuinely poolable files.

**GATES:** `winprob_rollout_weight_test.py` (26) + one `combination_checks` ARGVS row. **SMOKE**
(`--debug --steps 10000 --critic winprob --no-hand-shaping --terminal-indicator --victory-value 1.0
--draw-penalty 0 --cf-records --cf-records-keep 4096 --win-prob-rollout-target 0.03
--win-prob-rollout-r 4 --win-prob-rollout-weight 64 --win-prob-lambda 0.9`, rust bridge, CPU):
`Training complete`; 4 rollouts, 52–61 states labelled each, `rollout_weight` 64,
**`rollout_mass` 0.0265–0.0301 → `rollout_mass_weighted` 0.625–0.663** (the closed form
`f·k/(1+f(k−1))` gives 0.664 at f = 0.03, so the meter and the arithmetic agree),
...
## 2026-09-23 — TRAINING INPUT: `data/teams/sample/` is Smogon's 32 again; promoted teams get their own role (`gen3_curated_sample_split_v1`)

**THE TRAINING-INPUT CHANGE.** The default trainee builder (`Gen3Teambuilder(all_teams,
bias_teams=…, bias_prob=0.1)`, built by `main.train.matchup_setup`) now over-draws the **32 curated
Smogon teams** (`TeamLoader.get_training_bias_teams()`). Since the 2026-08-31 promotion it had
over-drawn all **72** teams then in `data/teams/sample/` — the 32 plus the 40 fleet teams
`python -m main.promote_teams` had copied in to make them legal `--exploiter` trainees. One folder was
doing two jobs. Per 10% bias draw, a curated team's probability goes from 1/72 to 1/32; a promoted
team's from 1/72 to 0. The 90% pool draw is unchanged. **The bias SET is not recorded in the
MatchupSpec** (adding it would move every run's `spec_hash`); a run's set is known from its pin commit.

**THE POOL: 719 → 719, one team swapped.** Moving the 40 files out of `sample/` changed nothing
(same shas, same order — `get_all_teams()` is `sample + promoted + other`, the pre-split order).
The re-sync then followed the thread, which now links a NEWER paste of Curse RestLax ForreDol:
`0972146213a667c9` (sha `45995e432f`) → `04e417ef9822abe9` (sha `1808014a9a`). The only difference:
the old paste spells out Dugtrio's `IVs: 30 SpD / 30 Spe` for Hidden Power Bug; the new one leaves the
importer to apply the default Bug IVs (`30 Atk / 30 Def / 30 SpD`) — a genuinely different Dugtrio
(1 point of Speed and of Attack/Defense). Both validate `gen3ou`; both played on the node and rust
bridges. The old paste is RETIRED to `data/teams/superseded/` (not in the pool; still a legal exploiter
trainee, because 19 archived runs pinned it — the K6 trio and the 2026-09-22 stall teamset among them).

**Roles, one per top folder** (`utils.team_loader.manifest_role`, replacing `"sample" in root`):
`sample/` (exactly the thread; `tools/sample_team_downloader` is its ONLY writer), `promoted/` (40 +
`PROMOTION_MANIFEST.{json,md}`), `superseded/` (1, outside the pool), anything else = `other`. New
accessors: `get_training_bias_teams`, `get_promoted_teams`, `get_superseded_teams`,
`get_exploiter_trainee_teams` (= sample + promoted + superseded, 73). The exploiter guard is now
`matchup_spec.validate_exploiter_trainee_is_vetted` over that set (was `…_is_sample` over
`get_sample_teams()`), with an error that no longer calls the set "tournament-proven".

**Relocations.** `data/teams/relocations.json` maps the 41 old `teams/sample/…` paths to their new
homes; `utils.team_loader.resolve_team_file` follows it ONLY for a path that no longer exists, never
guesses, and says so on stderr. Every reader of a RECORDED team path goes through it
(`MatchupSpec.from_args` — which still RECORDS the path as typed, so `spec_hash` does not move —
`read_recorded_trainee_teams`, the distill team bias, `distill_spec`'s path check, the untaught
meter's manifests, `play.py --team`). Measured on the archive: 92 runs with pinned trainee teams,
383 recorded paths (81 relocated), 0 unresolvable; all 72 guarded exploiter runs still validate;
60 distill runs' 76 explicit team files (18 relocated) all resolve.

**What did NOT move, by construction.** Goldens, benchmarks and the cross-run measurement
instruments read `utils.team_loader.pins.PRE_SPLIT_SAMPLE_72` (the pre-split `get_sample_teams()`,
by sha, index 17 = the superseded paste): the obs golden (`golden_obs_capture`, 991 decisions,
digest `41fc8cf9…` before and after), the reward golden, `record_fixture_battle`, the
`get_sample_teams()`-drawing benchmarks, and — as `measurement_bias_teams()` — the eval worker,
the snapshot ladder, the bot ELO calibration and the bot matchup matrix. So `eval/elo`,
`win_rate_vs_pool`, `ladder.json` and the bot anchors get NO team-regime boundary here, and the
ladder's reuse of eval-measured pairs stays valid. The price: eval's 10% bias draw is the frozen 72
while training's is the curated 32.

**The downloader** pairs each title with its OWN link from the post structure (not an ordered zip
of links and titles), ignores sprite-less and spoilered pokepast links, REFUSES a titleless team
link, a manifest holding any entry it did not write, a changed team count without
`--accept-count-change`, and any partial download; a dropped paste is retired with a relocation.
The re-sync's only metadata change besides Curse RestLax: Raikou Celebi Slop's author is now
"Shitrock enjoyer and giraffefromholland" (the old reader kept the first author only).

**GATES:** `tools/sample_team_downloader/sample_team_downloader_test.py` (15, on the saved first
post `fixtures/first_post_2026-09-23.html`), `src/utils/team_loader/team_roles_test.py` (10),
`src/main/train/training_bias_set_test.py` (3), `loader_test.py`'s 32/40/647/719 pin, and the
reworked `promote_teams_test.py` / `matchup_spec_test.py` (the 2026-08-31 draw reproduces on the
pool it ran on; `data/teams/promoted/` is exactly that draw).
