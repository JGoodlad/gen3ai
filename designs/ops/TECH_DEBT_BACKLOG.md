...
| P0 | **`reward_manager.py` is at 1,990 of the 2,000-line hard bound** — the same trap `ppo.py` was in; the next edit anywhere in it trips the gate. Decompose around the registry/fold contract (`reward_weights.py` and `reward_composition.py` are already split out; the next cut is the per-term fold functions vs `RewardConfig`) | a gate that is 10 lines from red blocks the next reward fix at the worst moment; the reward path is the one every arm's objective runs through | M |
...
| P1 | Stale numerics the census listed and nobody has swept: training leaf "`--eval-workers` (default 3)" vs parser default 5; rust_sim leaf "STILL NEEDED before rust replaces node in better_line" (wrong on 3 claims); the "40 teams" pool count | each is a fact an agent will act on | S |
