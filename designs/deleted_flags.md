...
| flag | citation | note |
|---|---|---|
| `--hand-shaping` | `gen3_shaped_reward_deletion_v1` (config v122, Rust Core deletion pass, program §4 M3 row, 2026-09-26) | the MASTER switch for every hand-designed dense term (the eight PBRS potentials + the whole BIAS class). The whole shaped reward path is deleted; the reward is the TERMINAL alone. A checkpoint that recorded it ON refuses to resume/fork (`agents.model.model_version.shaped_reward`) — run it pinned to ≤ `029cee83` |
| `--no-hand-shaping` | `gen3_shaped_reward_deletion_v1` (config v122, Rust Core deletion pass, program §4 M3 row, 2026-09-26) | the clean-world spelling of the above, which every win-prob-era argv carries; now simply unrecognised (and unneeded: terminal-only is the only reward). A recorded argv carrying it is judged by its pinned parser |
| `--all-shaping-pbrs` | `gen3_shaped_reward_deletion_v1` (config v122, Rust Core deletion pass, program §4 M3 row, 2026-09-26) | "everything but stall": folded Φ_hazard/Φ_boost/Φ_opp_boosts/Φ_status and zeroed every BIAS term but `no_progress_tax` |
| `--pbrs-material` | `gen3_shaped_reward_deletion_v1` (config v122, Rust Core deletion pass, program §4 M3 row, 2026-09-26) | the material potential Φ_mat. Its by-product, the normalised material MARGIN (the `win_margin` obs key), survives as `agents/training/material_margin.py` |
| `--pbrs-belief` | `gen3_shaped_reward_deletion_v1` (config v122, Rust Core deletion pass, program §4 M3 row, 2026-09-26) | the incoming-KO belief potential Φ_belief |
| `--stall-pbrs` | `gen3_shaped_reward_deletion_v1` (config v122, Rust Core deletion pass, program §4 M3 row, 2026-09-26) | the "stall" switch: folded Φ_progress and zeroed `no_progress_tax` + `stall_tax` |
| `--no-stall-pbrs` | `gen3_shaped_reward_deletion_v1` (config v122, Rust Core deletion pass, program §4 M3 row, 2026-09-26) | the generated negation of the above; named in `src/main/launcher/CLAUDE.md` as an example of a pinned commit's AST-resolved `--no-` spelling |
| `--bias-redesign` | `gen3_shaped_reward_deletion_v1` (config v122, Rust Core deletion pass, program §4 M3 row, 2026-09-26) | the staged BIAS redesign (the no-progress clock replacing the anti-spam taxes) |
| `--bias-additivity` | `gen3_shaped_reward_deletion_v1` (config v122, Rust Core deletion pass, program §4 M3 row, 2026-09-26) | the BIAS class's additive↔telescoping λ (accumulate-and-refund) |
| `--mat-alive-weight` | `gen3_shaped_reward_deletion_v1` (config v122, Rust Core deletion pass, program §4 M3 row, 2026-09-26) | Φ_mat's per-mon-alive weight; frozen at its 1.25 in `material_margin.MAT_ALIVE_WEIGHT` for the `win_margin` obs key |
| `--no-progress-penalty` | `gen3_shaped_reward_deletion_v1` (config v122, Rust Core deletion pass, program §4 M3 row, 2026-09-26) | the no-progress TAX magnitude. The `ProgressClock` survives as the OBS-only `turns_since_progress` counter; its `last_penalty` half is deleted |
| `--arm-no-progress-tax` | `gen3_shaped_reward_deletion_v1` (config v122, Rust Core deletion pass, program §4 M3 row, 2026-09-26) | re-armed `no_progress_tax` under `--no-hand-shaping` (the win-prob era's anti-stall contingency; never ON in production) |
| `--switch-bias-weight` | `gen3_shaped_reward_deletion_v1` (config v122, Rust Core deletion pass, program §4 M3 row, 2026-09-26) | the belief-risk-scaled stay-tax / escape BIAS lever |
| `--self-ko-hp-penalty` | `gen3_shaped_reward_deletion_v1` (config v122, Rust Core deletion pass, program §4 M3 row, 2026-09-26) | the HP-scaled self-KO BIAS penalty |
| `--drop-redundant-bias` | `gen3_shaped_reward_deletion_v1` (config v122, Rust Core deletion pass, program §4 M3 row, 2026-09-26) | zeroed `stall_tax` + `matchup_penalty` |
| `--drop-switch-bias` | `gen3_shaped_reward_deletion_v1` (config v122, Rust Core deletion pass, program §4 M3 row, 2026-09-26) | zeroed the hand-coded switch-subsidy BIAS family |
