...
| H3 | **Surprise-OHKO / unrevealed-threat** obs-coverage gap | 🔬 | Belief UNDER-FIRES on 52% of lethal healthy-stay deaths (pko<0.3). Provenance: 36% just-switched, 42% new-move (priors gap), 22% fully-known (calibration bug). CAVEAT: ~same rate in wins (56%) → pervasive. Recoverability PENDING. | `models/saved_work/surprise_death_recoverability.py` |
