...
| D4 | The N=20 exploiter ceiling is **conditioning-signal starvation** (not capacity) | ❌ **BOTH ROUTES SAY NO** (arm 3 running) (2×2, `designs/ai_v8/design_conditioning_ceiling_arms.md`) | Count sweep N=1 0.84 / N=3 0.835 / N=10 0.825 / **N=20 stalls ~0.66**. FiLM works but under-powered (far-z range 0.0153 vs 0.048–0.136 needed); z is COMPOSITIONAL (z↔behaviour r=0.125); the limiter measured as SNR/ill-conditioning, not capacity. **Now under test** as a 2×2 over LUT × team-diversity, all arms byte-identical to the `ai_v8_12` baseline but for one field: arm1 def-20+LUT (the LUT effect), arm2 random-20 no-LUT (the diversity effect — win-rate matched 0.548 vs 0.547, but z participation ratio 9.59 vs 6.30, ×1.52), arm3 random-20+LUT (do they compose). Gate: `tmp/lut_verdict.py` pools ≥4 plateau cycles vs baseline 0.653 / N=10 ceiling 0.72. **RESULTS (2026-07-27) — TWO INDEPENDENT ROUTES TO MORE CONDITIONING SIGNAL, BOTH NULL.** Plateau win-rates vs the frozen target (pooled ≥4 cycles from +7M; baseline def-20 no-LUT **0.6488**, N=10 ceiling **0.7250**): **arm 1** def-20 **+LUT** = **0.6725** ±0.033 → effect **+0.024, CI [−0.016, +0.064]** (n.s.); **arm 2** **random-20** no-LUT (win-rate matched 0.548 vs 0.547, ×1.52 the z code rank) = **0.6225** ±0.034 → effect **−0.026, CI [−0.067, +0.015]** (n.s., and pointing the WRONG way). Both arms' CI upper bounds sit below the 0.72 target ⇒ **GAP NOT CLOSED** either way. A free per-team code is the MAXIMUM conditioning signal the architecture can take (canary `lut_hit_frac` 1.0, `lut_code_dist` 1.0, 20/20 teams addressed) and it did not clear the stall. ⇒ **the conditioning-signal theory is UNSUPPORTED; do NOT climb to LoRA/MoE on it.** **Honest limits:** neither effect is SIGNIFICANT, so the claim is "no route produced a detectable gain", NOT "conditioning provably does nothing"; the ±0.03 null band needs ~5,000 games (≈50M steps) to emit a true DECISIVE NULL, so both residuals are UNRESOLVED; and arm 2 vs baseline is across DIFFERENT team sets (matched on team STRENGTH, which is not the same as how exploitable the target is with those teams). **Arm 3** (random-20 **+LUT**) removes that last confound — vs arm 2 it is a WITHIN-team-set LUT isolation; **arm 4** (def-20 +LUT, ZERO-init) reruns arm 1 without its −0.040 fork handicap. | `tmp/lut_verdict.py`, `tmp/z_spread_compare.py` |
