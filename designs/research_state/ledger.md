...
| D4 | The N=20 exploiter ceiling is **conditioning-signal starvation** (not capacity) | ❌ **LUT ARM SAYS NO** (arm 2 running) (2×2, `designs/ai_v8/design_conditioning_ceiling_arms.md`) | Count sweep N=1 0.84 / N=3 0.835 / N=10 0.825 / **N=20 stalls ~0.66**. FiLM works but under-powered (far-z range 0.0153 vs 0.048–0.136 needed); z is COMPOSITIONAL (z↔behaviour r=0.125); the limiter measured as SNR/ill-conditioning, not capacity. **Now under test** as a 2×2 over LUT × team-diversity, all arms byte-identical to the `ai_v8_12` baseline but for one field: arm1 def-20+LUT (the LUT effect), arm2 random-20 no-LUT (the diversity effect — win-rate matched 0.548 vs 0.547, but z participation ratio 9.59 vs 6.30, ×1.52), arm3 random-20+LUT (do they compose). Gate: `tmp/lut_verdict.py` pools ≥4 plateau cycles vs baseline 0.653 / N=10 ceiling 0.72. **ARM 1 RESULT (2026-07-27): a FREE per-team code moved +0.024, CI [−0.016,+0.064] — NOT distinguishable from zero, and DECISIVE POSITIVE ruled out (CI upper 0.705 < the 0.72 target). Max conditioning signal, no gap closed ⇒ the conditioning-signal theory is UNSUPPORTED; do NOT climb to LoRA/MoE on it.** Caveat: the ±0.03 null band needed ~50M steps to emit DECISIVE NULL, so the +0.024 residual is UNRESOLVED, not proven zero. Arm 2 (diversity) now tests whether z-crowding is the limiter at all. | `tmp/lut_verdict.py`, `tmp/z_spread_compare.py` |
