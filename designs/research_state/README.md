...
| **De-amortization: per-team conditioning (z_arch/FiLM → LUT)** | The generalist plays every team with one averaged policy; a specialist on the SAME team is far stronger (piloting 0.438 vs 0.72) — the amortization gap, and the largest *measured* headroom we have. The exploiter→distill loop CLOSES it (ledger **D1**: ELO 1986→2055, piloting 0.438→0.710) and the skill STICKS without teachers (**D2**: ~76% retained at equilibrium). Open question is SCALE: N=1/3/10 teachers all distil cleanly, **N=20 stalls** (**D4**) — FiLM's conditioning path is SNR/ill-conditioning-limited, not capacity-limited (far-z dynamic range 0.0153 vs the 0.048–0.136 needed; z↔behaviour r=0.125). | 🔬 **ACTIVE** — next test = a per-team **LUT** embedding on the def-20 cluster (`tmp/pool_cluster_n20/`): if a free per-team code clears the stall, the ceiling is conditioning SIGNAL, not the idea. `designs/ai_v8/impl_step_retention_ablation.md`, `designs/learning/conditioning_architectures.md` §5b |
...
symptoms, not causes). The blunder-hunting vein is largely mined out. **Since 2026-07 the active vein is
de-amortization** (the last row) — the one lever with a measured gap, a working mechanism, and a
confirmed *durable* payoff; its open ceiling is the N=20 conditioning-signal question, not the idea.
