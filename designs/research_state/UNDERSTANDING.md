...
25. **A leaf read needs ≥400 paired mirror games; a 100-pair cell cannot resolve a 4-point effect, and the direct metric is PAIRWISE ACCURACY on contested sibling forks under shared dice.** The identical control cell read 0.495 and 0.535 in two windows (paired +0.040 [−0.001, +0.081]); the second mechanically prints a lower bound above 0.50. Pairwise accuracy (chance 0.50; the promoted head 0.517; a head refit on counterfactual successors 0.603) is a forward pass per pair and is read BEFORE any battle is spent. [ledger 2026-09-14 · *MEASUREMENT (MAJOR) · SIBLING DISCRIMINATION*]
...
[MEASURED (MAJOR), `paired_refit_discrimination_2026-09-14`] **The promoted win-prob head ranks successor states one move
apart AT CHANCE: pairwise accuracy 0.517 [0.480, 0.552] on 562 held-out non-tied contested pairs under shared dice —
the first direct measurement of the property the one-ply test consumes, and the arithmetic under every null leaf.**
Refitting only the head on a FROZEN `ctrl10M` trunk with plain BCE on counterfactual successors lifts it to **0.603
[0.569, 0.637]** (+0.086 DETECTED); adding a pairwise ranking term buys nothing (−0.011, n.d. at every coefficient).
So the trunk holds the ordering and the on-policy stream never asked the head for it: **the data is the fix, not the
loss form.** The refit head does not pay as a leaf on 100-pair cells (rung B −0.0125, `grid` −0.0125, n.d.) — and a
100-pair L2 cell cannot resolve a 4-point effect (its own control moved +0.040 between windows). Three more heads read
(`rollout`, `ent05`, `vf025`): nine win-prob heads, no usable leaf. Policy top-1 and top-2 are outcome-interchangeable
on contested decisions (0.708 / 0.708); a random third branch costs 2.9 pp with a 4.5 % blind-spot rate. The
`cflabels` factory paired the dice but not the policy draws — a concrete account of its null. **Next arm: forks INTO
training** (contested states, top-2 + random, CRN on dice and draws, plain BCE), read first on pairwise accuracy
against 0.517, then the battery at ≥400 pairs. [ledger 2026-09-14 · *MEASUREMENT (MAJOR) · SIBLING DISCRIMINATION*] **The `grid` cell is the sensitive leaf row instead:** `strata` 0.315 and `denseaux` 0.303 are the best of six, `cflabels` 0.188 the worst (strata − cflabels +0.128 [+0.057, +0.198] DETECTED) — the successor-discrimination head is the most confident re-ranker (68 % of actions changed) and the most wrong, a null reported DOSE-UNREAD (`cf_head_only`, 150k-step label lag, realized duty cycle never read), not a verdict on counterfactual labels as a class. [ledger 2026-09-12 · *MEASUREMENT (MAJOR) · THE LEAF BATTERY, PHASE 2/3*]
