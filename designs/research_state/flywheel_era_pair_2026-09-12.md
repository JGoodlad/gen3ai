...
## 9e. STATUS 2026-09-20 (later) — the LOSS is the carrier; the plateau plan

The split (`split_lossoff_read_2026-09-20/`) exonerates fold-1's ecology bundle — team bias 0.4, two specialists in the pool, and the fourth lever `--team-block-episodes 64` that no registration had named — on 9 of 9 cells; the top-1 action-target loss carried the whole cost. Externally (`anchor_ab_continuation_2026-09-20/`) the continuation's +15.5 pp is ~3 pp vs Metamon SmallRL (n.d.) while the fold's −0.08 vs its parent reproduces. **Owner plan (2026-09-20):** (1) `ai_v13_12_plateau` — continue the continuation from 87.1M in 8M blocks, frozen dose, stop when a block's untaught Δ is WITHIN the 3.69 floor, confirm with a 1,200-game SmallRL anchor on the Rust front end; (2) three 5-team exploiters (one per archetype) forked from the PLATEAU PARENT, admitted only if OUTSIDE the floor above it on their own five teams; (3) three folds from the plateau parent differing only in `--distill-topk 1 / 3 / 11` (`--distill-target action`, `--distill-beta 1.0`, `--distill-gate none`), on fold-1's full ecology (measured inert), +12M each, read on untaught + the three slices vs the plateau parent. Seed 1002 control last. Every fold on this side is read against a continuation (or a plateaued parent), never the frozen parent.
