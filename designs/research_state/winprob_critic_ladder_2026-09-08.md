...
|---|---|---|---|---|---|---|
| `ai_v12_10_ladder_vf15` | `--vf-coef 1.5` (v6 2026-09-11: class AUC t4–10 **−0.082 / −0.080 DETECTED DOWN on two draws**; the decodable part −0.07 / −0.06 — more value gradient REMOVED opponent information from the representation — CONFIRMED at the representation level by `repr_class_decode_2026-09-11` at HALF the V effect and NOTHING at t1–3; UNREPLICATED at run level; replicate `vf15_b` GO 2026-09-12, registered PASS = Δ CI clears −0.022 at both 800-game draws vs all three controls) | +0.0140 [−0.0080, +0.0389] | +0.0486 [−0.0305, +0.1311] | +0.1634 [−0.0807, +0.3733] | **NOT DETECTED** on all four; identity bias on ALL states +0.048 [+0.014, +0.082] MORE optimistic (vs ZERO, no floor) | `measurements/critic_ladder_reads/vf15_vs_ctrl10M_2026-09-08/` · ledger 2026-09-08 *READ · critic ladder arm 1* |
...
| **`ai_v12_17_ladder_strata`** | `--win-prob-strata-weight 1.0` (realized bot share 0.473 vs 0.5) | +0.0052 [−0.0121, +0.0161] | −0.0153 [−0.0929, +0.0585] | −0.0047 [−0.2340, +0.1806] | NOT DETECTED on every row registered at the time. **v6 (2026-09-11), on the decision row declared before the read: class AUC t4–10 0.760 vs 0.710, Δ +0.050 [+0.036, +0.064], DETECTED past the 0.022 floor — the campaign's FIRST detection on a decision row; t1–3 +0.040 DETECTED; THREE draws: +0.050 / +0.033 / +0.044, two of three clearing the floor by CI. **REPLICATE `strata_b` (seed 1002, same pin) FAILED 2026-09-12: 0.679 on both draws, −0.029 / −0.034 / −0.008 vs the three controls — NOT CONFIRMED; within-lever spread 0.08 = 3.6× the floor** (`critic_ladder_reads/strata_b_vs_ctrl10M_2026-09-12/`)** | `measurements/critic_ladder_reads/strata_vs_ctrl10M_2026-09-09/` · ledger 2026-09-09 *READ · critic ladder arm 7* |
| **arm 8 — `ai_v12_19_ladder_lambda09`, `--win-prob-lambda 0.9`** (pin `28ece02a`) | READ live + offline 400/800 | bot +0.002 (400) / **+0.010 [+0.005, +0.015]** (800) | WITHIN FLOOR | WITHIN FLOOR | **CANDIDATE WITHDRAWN** — the live-frame own-team +0.15 does not replicate offline (0.04–0.06 vs 0.04–0.05); **between-opponent spread ratio DOWN past the floor at both sizes (Δ −0.12 / −0.10)** = shrinkage (C), pending the calibration slope; resolution leans up; dose curve {1.0, 0.95, 0.9} registered | `measurements/critic_ladder_reads/lambda09_vs_ctrl10M_2026-09-10/` · ledger 2026-09-10 *arm 8 on the offline frames* |
...
| **arm 9 — `ai_v12_20_ladder_denseaux`, `--win-prob-dense-aux 1.0`** (pin `46ca68ef`) | READ live vs all three controls | +0.0072 / −0.0030 / +0.0082 (inside the 0.010 floor) | inside the floor | n.d. | **NOT DETECTED on every row**; spread ratio leans UP (0.151 vs 0.09–0.12, n.d.); own-team live-frame rows not read (rule 18); slope 1.45 inside the run floor. **OFFLINE 400/800 vs all three controls (2026-09-11): nothing on any registered row; class AUC t4–10 (v6) 0.714 = the roster's highest point, inside the 0.022 floor vs every control; the live spread UP-lean is absent offline — arm CLOSED** | `measurements/critic_ladder_reads/denseaux_vs_ctrl10M_2026-09-10/` · ledger 2026-09-10 *READ · arm 9* |
| **`ai_v12_21_ladder_lambda09_b` — arm 8's RUN-LEVEL REPLICATE** (seed 1002, pin `28ece02a`) | READ offline 400/800 vs three controls | resolution all +0.004–0.006, every CI covers zero | within floor | within floor | **PRE-REGISTERED TEST FAILED — the λ-0.9 overall-resolution candidate is NOT CONFIRMED** (arm 8 +0.009–0.011 clear; replicate same sign, half size). Pair profile: amplitude DOWN vs the base control; own-team R² t1 UP ~0.02 at the floor (both draws); class AUC t4–10 at or BELOW every control (0.705 / 0.693) | `measurements/critic_ladder_reads/lambda09_b_vs_ctrl10M_2026-09-11/` · ledger 2026-09-11 *READ · the λ-0.9 replicate* |
| **arm 11 — `ai_v12_22_ladder_lambda095`, `--win-prob-lambda 0.95`** (seed 1002, pin `28ece02a`) | READ offline 400/800 vs three controls | within floor | within floor | n.d. | **REGISTERED DIRECTIONAL CLAIM FAILED** (t1–3 spread below each control with CI < 0: 3 of 6; the point below all six). Dose curve at 800: 1.0 → 0.09–0.15, **0.95 → 0.084**, 0.9 → 0.05 — monotone by points, only the 0.9 rung distinguishable. Class AUC t4–10 0.697 vs 0.713: the λ family leans DOWN at every dose (n.d.). λ CLOSED for conditioning | `measurements/critic_ladder_reads/lambda095_vs_ctrl10M_2026-09-11/` · ledger 2026-09-11 *READ · arm 11* |
| **arm 10 — `ai_v12_23_ladder_rollout`, `--win-prob-rollout-target` + `--win-prob-rollout-weight 64`** (built `aa6822b2` + `40bf23d9`, v119) | **READ (2026-09-11): NOT DETECTED on every primary row at both draws vs both λ draws — class AUC t4–10 −0.017 / −0.006 at 800, resolution within floor; dose delivered 7.09 % at all 98 updates; the richer-label family is CLOSED at 10M** (W = 64 anchors-only, ESS 19.6 %; increment 46ca68ef→40bf23d9 NEUTRAL); comparator = the λ-0.9 PAIR with its crossing mismatch stated (draw 1 seeded self-play at 2.16M, draw 2 at 4.13M — a fraction ramp from ~3 %: cumulative self-play exposure 63.6 % vs 63.0 % of training, a 0.6 pp difference; the discrete asymmetry is one extra seeded snapshot); SECONDARY = BOTH single-draw reads, none selected by the post-treatment crossing | — | — | — | **the requested 1/32 fraction is ~26× the run's whole simulation budget; the 1× fraction is 1/832 (0.0012, R=8) = 0.12 % of the loss mass — a null by arithmetic.** Redesign in force: `--win-prob-rollout-weight` (anchored rows ×32, ~4 % mass) being built; the arm moves onto arm 8's λ 0.9 argv (seed 1003) and is read against the λ-0.9 pair; launch on hash + increment verdict, after `lambda095` | ledger 2026-09-10 *BUILT + COST FINDING · arm 10* |
| `ai_v12_14_ladder_truevalue` (arm 5, PRIVILEGED critic, pin `d11386dc`) | `--value-true-team` | +0.0138 [−0.0044, +0.0315] | +0.1076 [+0.0255, +0.1875] (n.d. vs floor) | +0.2068 [−0.0250, +0.4162] | **NOT DETECTED on the decision rows — the opponent axis did not move (spread ratio 0.00 vs 0.09–0.12): hidden information is not the limit**; the privileged head is more OPTIMISTIC late (+0.088 vs ≈0) against all three controls; pool resolution down. **OFFLINE 400/800 (2026-09-10): bot resolution +0.000 / +0.003 WITHIN FLOOR (the live lean gone), spread ratio −0.045 / −0.028 and own-team R² −0.018 / −0.014 both DOWN inside the floor, optimism +0.08–0.09 and ECE all +0.04 on every draw (both POST-HOC leans; no draw 2 exists) — CEILING PROBE CLOSED** | `measurements/critic_ladder_reads/truevalue_vs_ctrl10M_2026-09-10/` · ledger 2026-09-10 *READ · arm 5* |
...
## 2b′. STATUS 2026-09-12 — after `strata_b`, the representation decode, the leaf battery and the step curve

| question | answer | where |
|---|---|---|
| does any lever move the decision row `cond.opp_class_auc.t4_10`? | **ONE, downward.** `vf15` −0.082 and its replicate `vf15_b` −0.053…−0.087 (six of six CIs clear the floor) — `--vf-coef 1.5` CONFIRMED to strip class information from V, resolution unchanged. `strata` +0.050 → replicate −0.03: NOT CONFIRMED. Every other lever within floor. **`vf 0.25` NOT DETECTED (−0.034…+0.003, five of six within floor): the lever is NON-MONOTONE — flat from 0.25 to 0.5, falls at 1.5; 0.5 stays** | ledger 2026-09-12 *VERDICT · vf15_b PASSES*, *VERDICT · strata_b FAILS*; 2026-09-13 *VERDICT · vf025 NOT DETECTED* |
| is the row a head or a representation quantity? | **both move together** (branch A) — `strata` +0.048/+0.064 and `vf15` −0.052/−0.045 on `value_pooled → class`, same signs as V; the V row stays the decision row (cheaper, tighter floor). `strata_b`'s trunk is AT the controls (+0.007/+0.016) and only its V is below — the strata representation claim does not replicate either; seed-to-seed variance is head-concentrated (Δpooled/ΔV 0.51/0.57) | `measurements/repr_class_decode_2026-09-11/`, `repr_class_decode_strata_b_2026-09-12/` |
| do more steps move it? | **no.** 10M→73M on the 75M run: resolution.bot flat, class decode flat-to-down (a 20→40M step DOWN), ECE 0.102 → 0.017 — steps buy calibration | `measurements/winprob_step_curve_2026-09-11/` |
| does any lever move leaf quality (the search path's row)? | **no.** Six heads on the L2 null; L1 convicted as a width meter (rule 23); the `grid` cell separates heads (strata/denseaux best, cflabels worst, dose-unread) | `measurements/search_dividend_winprob_heads_2026-09-11/` |
| does any lever move STRENGTH at 10M? | **none outside the 45-Elo floor** (fifteen arms, matched count 4); `vf15_b` / `strata_b` downward candidates only; the four-node ladder cannot resolve its own floor (CI95 ±43–48). Seed replicates differ by 58–83 Elo vs the control pair's 10 (rule 22 on strength) | `measurements/ladder_strength_table_2026-09-12/` |
| the shaped-critic controls | `ctrl10M_shaped` (terminal-only, scalar MSE, gamma 0.9999, head as auxiliary) NEVER CROSSED — 0.77 vs bots at 10M, VOID for the conditioning row, a training-DYNAMICS finding; `ctrl10M_shaped_dense` (1 TERMINAL + 7 PBRS + 1 BIAS) ALSO never crossed (0.531 vs bots at 10M) and breached G7 (1.52). **Both VOID for conditioning, and they license NO statement about the shaped objective: they differ from the era's shaped critic in SIX resolved settings (PopArt off, no distributional readout, a 20× heavier auxiliary head, gamma, the head's role, the critic) — the era's own shaped fresh run `ai_v9_29_rev1` crossed at 2,000,016 with 0.74 vs bots.** The flywheel pair's shaped arm runs the ERA's shaped configuration | ledger 2026-09-12 *VERDICT · vf15_b PASSES* (c)–(e); *READ · the shaped-critic family at the LADDER argv* |
| does `--ent-coef 0.05` reproduce v8's entropy regime? | **YES** (`ent05`, arm 18): H_end 1.086 inside v8's 1.07–1.11 plateau, still rising at 10M; crossing/G7/bots in family. The flywheel pair runs at 0.05 in both arms. Conditioning row: NOT DETECTED at the registered standard, upward CANDIDATE (+0.014…+0.057, one of six past the floor) | ledger 2026-09-12 *VERDICT · ent05 PASSES*; *READ · ent05's conditioning row* |
| next arm | `vf15_b` (default under the 15-minute rule; the mechanism the detached-head question rests on); then `vf 0.25` if confirmed; if reversed, a matched-pin SHAPED-critic 10M CONTROL read on the same rows | ledger 2026-09-12 |
