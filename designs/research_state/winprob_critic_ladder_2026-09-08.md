...
| **arm 9 — `ai_v12_20_ladder_denseaux`, `--win-prob-dense-aux 1.0`** | **BUILT `46ca68ef`**, pin it; after `truevalue` → `lambda09_b` (launch gated on the increment test) | KataGo-style dense auxiliary head on `value_pooled`: per-slot survival + final HP + turns-to-end, unrevealed opponent slots masked; read on bot resolution + own-team R² (+ within-team rows), identity as the guard | — | — | — | registered; ledger 2026-09-10 *BUILT + REGISTRATION · arm 9* |
