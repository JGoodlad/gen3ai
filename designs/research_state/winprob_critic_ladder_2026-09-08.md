...
| the shaped-critic controls | `ctrl10M_shaped` (terminal-only, scalar MSE, gamma 0.9999, head as auxiliary) NEVER CROSSED — 0.77 vs bots at 10M, VOID for the conditioning row, a training-DYNAMICS finding; `ctrl10M_shaped_dense` (1 TERMINAL + 7 PBRS + 1 BIAS) ALSO never crossed (0.531 vs bots at 10M) and breached G7 (1.52). **Both VOID for conditioning. Not a head verdict: the era's own shaped fresh run `ai_v9_29_rev1` crossed at 2,000,016** — the failure is the ladder argv with the mode flipped. The flywheel pair's shaped arm runs the ERA's shaped configuration | ledger 2026-09-12 *VERDICT · vf15_b PASSES* (c)–(e); *READ · the shaped-critic family at the LADDER argv* |
| does `--ent-coef 0.05` reproduce v8's entropy regime? | **YES** (`ent05`, arm 18): H_end 1.086 inside v8's 1.07–1.11 plateau, still rising at 10M; crossing/G7/bots in family. The flywheel pair runs at 0.05 in both arms. Conditioning read running | ledger 2026-09-12 *VERDICT · ent05 PASSES* |
