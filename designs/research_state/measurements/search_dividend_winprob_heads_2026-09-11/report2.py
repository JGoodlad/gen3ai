...
#: The leaf-quality meter registered in the ledger on 2026-09-11. L1 = separation-of-raced (the
#: mechanism row), L2 = the paired mirror win rate (the outcome row, null 0.50 STRUCTURAL). The L1
#: bar is the win-prob family's maximum (15.0%) plus the 3.1 pp within-family floor; the addendum
#: the same day makes a clear at <= ~20% PROVISIONAL, because the floor is the RANGE of three
#: points and is therefore biased small. The floor demotes, never promotes.
L1_BAR = 0.181
L1_PROVISIONAL_CEILING = 0.20


def l1_verdict(sep_rate):
    """The L1 row's verdict against the registered bar, with the provisional rule applied."""
    if sep_rate is None:
        return "-"
    if sep_rate <= L1_BAR:
        return f"below bar ({sep_rate:.3f} <= {L1_BAR})"
    if sep_rate <= L1_PROVISIONAL_CEILING:
        return f"CLEARS, PROVISIONAL ({sep_rate:.3f}, <= {L1_PROVISIONAL_CEILING})"
    return f"CLEARS ({sep_rate:.3f})"
...
def main() -> None:
    ...
    heads = ["ctrl10M", "lambda09", "wp73M", "strata", "denseaux", "cflabels",
             "anchor10M", "anchor10Mb"]
    ...
    print()
    print("L1/L2 LEAF-QUALITY METER (ledger 2026-09-11) — L1 bar > 0.181, <= 0.20 is PROVISIONAL; "
          "L2 bar = paired CI lower bound > 0.50")
    print(f"{'cell/head':26} {'L1 sep/raced':>12} {'L1 verdict':34} {'L2 paired':>10} "
    ...
    for k, r in out["cells"].items():
        ...
        print(f"{k:26} {str(sep):>12} {l1_verdict(sep):34} {pw:>10} {ci:>18} {l2:22}")
    print()
