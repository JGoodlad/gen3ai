#!/usr/bin/env bash
# ONE head-to-head SESSION: N games, arm W's checkpoint (fixed pool team) accepting
# challenges from Foul Play (random pool team per battle).
# Verbatim from arm S's run_session.sh but for the model, the usernames and the root.
#
...
PORT=9417
MODEL=$GEN3AI/models/ai_v13_02_flywheel_winprob/final_model.zip
# 🚨 KEEP THESE UNDER 19 CHARACTERS (anchors SOP hazard H1).
OURS=G3aiW916s${SESSION}
THEIRS=FpW916s${SESSION}
