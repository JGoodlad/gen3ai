...
WT=/home/goodlad/dev/gen3ai-wt/metamon-matched
...
if [ "$REGIME" = "greedy" ]; then OUR_TEMP=0.0; else OUR_TEMP=1.0; fi
...
start_metamon() {
  ...
  PYTHONUNBUFFERED=1 METAMON_CACHE_DIR=/home/goodlad/dev/metamon/cache \
    ...
    --role "$1" --total_battles "$N" "${META_TEAMS[@]}" --regime "$REGIME" \
...
{"agent":"$AGENT","regime":"$REGIME","team_set":"$TEAMS","challenger":"$WHO",
 ...
 "our_temperature":$OUR_TEMP,"our_team_seed":$OUR_SEED,"their_team_seed":$THEIR_SEED,"tag":"$TAG"}
JSON
