...
The critic ladder's registered read (ledger 2026-09-08 *REGISTRATION · THE CRITIC LADDER*), produced by `python -m main.ops.critic_read` v6 at 2026-09-11T11:27:37. Every number is a DELTA, **arm − control**, with the difference of the two runs' independent battle-clustered bootstraps. Strength is NOT read.
...
|---|---|---|---|---|---|---|
| resolution · `bot` | — | +0.0173 | +0.0170 | **+0.0003** | [-0.0044, +0.0055] | **WITHIN FLOOR** (|delta| <= floor 0.0074) |
...
| between-opponent spread ratio sd(V)/sd(outcome), noise-corrected · `turn 1-3` | as traced | +0.1241 | +0.1454 | **-0.0213** | [-0.0507, +0.0054] | **WITHIN FLOOR** (|delta| <= floor 0.0533) |
...
| calibration SLOPE — weighted logistic regression of the outcome on logit(V) (1 = correctly dispersed, >1 = SHRUNK, <1 = over-dispersed) · `all states, <=2 per battle` | as traced | +1.2211 | +1.0695 | **+0.1516** | [+0.0731, +0.2303] | **NOT DETECTED** (CI does not clear the floor 0.1085) |
...
|---|---|---|---|---|---|---|---|---|
| resolution ⭐ | `all` | `capture-rate (gauge)` | +0.0383 | +0.0386 | **-0.0003** | [-0.0062, +0.0058] | 400 | **WITHIN FLOOR** (|delta| <= floor 0.0003) |
| resolution ⭐ | `bot` | `capture-rate (gauge)` | +0.0173 | +0.0170 | **+0.0003** | [-0.0044, +0.0055] | 400 | **WITHIN FLOOR** (|delta| <= floor 0.0074) |
| resolution ⭐ | `pool` | `capture-rate (gauge)` | +0.0564 | +0.0614 | **-0.0050** | [-0.0148, +0.0070] | 400 | **WITHIN FLOOR** (|delta| <= floor 0.0141) |
...
| ece | `all` | `capture-rate (gauge)` | +0.0265 | +0.0237 | **+0.0028** | [-0.0074, +0.0139] | 400 | **WITHIN FLOOR** (|delta| <= floor 0.0144) |
...
| Murphy reliability | `ALL` | `pop` | +0.0034 | +0.0030 | **+0.0004** | [-0.0036, +0.0047] | 4000 | **WITHIN FLOOR** (|delta| <= floor 0.0022) |
| Murphy reliability ⭐ | `ALL` | `ipw` | +0.0034 | +0.0030 | **+0.0004** | [-0.0036, +0.0047] | 4000 | **WITHIN FLOOR** (|delta| <= floor 0.0022) |
...
|---|---|---|---|---|---|---|---|---|
| between-opponent spread ratio sd(V)/sd(outcome), noise-corrected ⭐ | `turn 1-3` | as traced | +0.1241 | +0.1454 | **-0.0213** | [-0.0507, +0.0054] | 2000 | **WITHIN FLOOR** (|delta| <= floor 0.0533) |
| the same ratio UNCORRECTED and unclamped | `turn 1-3` | MATCHED 797/334/9 | +0.1262 | +0.1476 | **-0.0214** | [-0.0481, +0.0037] | 2000 | **WITHIN FLOOR** (|delta| <= floor 0.0535) |
| sd(V) - sd(outcome), noise-corrected | `turn 1-3` | as traced | -0.0990 | -0.1018 | **+0.0027** | [-0.0111, +0.0164] | 2000 | **WITHIN FLOOR** (|delta| <= floor 0.0071) |
...
| between-opponent spread ratio sd(V)/sd(outcome), noise-corrected | `turn 4-10` | as traced | +0.4659 | +0.5080 | **-0.0421** | [-0.1081, +0.0238] | 2000 | **WITHIN FLOOR** (|delta| <= floor 0.2179) |
| between-opponent spread ratio sd(V)/sd(outcome), noise-corrected | `turn 11-24` | as traced | +0.6579 | +0.7039 | **-0.0460** | [-0.1420, +0.0484] | 2000 | **WITHIN FLOOR** (|delta| <= floor 0.1920) |
...
| opponent-CLASS (pool vs bot) AUC of V | `turn 1-3` | MATCHED 797/334/9 | +0.5848 | +0.5790 | **+0.0058** | [-0.0106, +0.0221] | 2000 | **WITHIN FLOOR** (|delta| <= floor 0.0153) |
| opponent-CLASS (pool vs bot) AUC of V | `turn 4-10` | MATCHED 797/334/9 | +0.7213 | +0.7103 | **+0.0111** | [-0.0033, +0.0244] | 2000 | **WITHIN FLOOR** (|delta| <= floor 0.0219) |
| OPPONENT-DECODABLE between-opponent spread ratio — sum_o q(o|V)*p_o, noise-corrected (NOT a bound on the row above) | `turn 1-3` | MATCHED 797/334/9 | +0.0203 | +0.0178 | **+0.0025** | [-0.0009, +0.0060] | 2000 | **PROVISIONAL — no floor; never DETECTED** (a DESCRIPTIVE DECOMPOSITION TERM, never a pass/fail bar and never a treatment effect. It is the between-opponent spread ratio a head conditioning ONLY on WHICH OPPONENT its own `V` reveals would exhibit — `sum_o q(o|V)*p_o` — and it is read BESIDE `cond.spread_ratio.<window>` to split that row into an OPPONENT-IDENTITY part and a remainder. 🚨 IT IS NOT AN UPPER BOUND ON THAT ROW AND `V` ROUTINELY EXCEEDS IT: measured on the hp800 lambda09/ctrl10M pair, V reads 0.647 / 0.704 at turns 11-24 against this row's 0.214 / 0.187. A per-state conditional mean is an ATTENUATING transform, so the cell-mean spread of `E[p_o | V]` is smaller than the cell-mean spread of `V` itself; the excess is between-opponent spread that rides on BOARD STATE correlated with the opponent rather than on opponent identity. A delta between two sides is a difference in how much opponent IDENTITY each side's output carries; it is reported, and it is never DETECTED) |
| OPPONENT-DECODABLE between-opponent spread ratio — sum_o q(o|V)*p_o, noise-corrected (NOT a bound on the row above) | `turn 4-10` | MATCHED 797/334/9 | +0.1314 | +0.1262 | **+0.0052** | [-0.0107, +0.0215] | 2000 | **PROVISIONAL — no floor; never DETECTED** (a DESCRIPTIVE DECOMPOSITION TERM, never a pass/fail bar and never a treatment effect. It is the between-opponent spread ratio a head conditioning ONLY on WHICH OPPONENT its own `V` reveals would exhibit — `sum_o q(o|V)*p_o` — and it is read BESIDE `cond.spread_ratio.<window>` to split that row into an OPPONENT-IDENTITY part and a remainder. 🚨 IT IS NOT AN UPPER BOUND ON THAT ROW AND `V` ROUTINELY EXCEEDS IT: measured on the hp800 lambda09/ctrl10M pair, V reads 0.647 / 0.704 at turns 11-24 against this row's 0.214 / 0.187. A per-state conditional mean is an ATTENUATING transform, so the cell-mean spread of `E[p_o | V]` is smaller than the cell-mean spread of `V` itself; the excess is between-opponent spread that rides on BOARD STATE correlated with the opponent rather than on opponent identity. A delta between two sides is a difference in how much opponent IDENTITY each side's output carries; it is reported, and it is never DETECTED) |
| OPPONENT-DECODABLE between-opponent spread ratio — sum_o q(o|V)*p_o, noise-corrected (NOT a bound on the row above) | `turn 11-24` | MATCHED 797/334/9 | +0.1709 | +0.1867 | **-0.0158** | [-0.0383, +0.0072] | 2000 | **PROVISIONAL — no floor; never DETECTED** (a DESCRIPTIVE DECOMPOSITION TERM, never a pass/fail bar and never a treatment effect. It is the between-opponent spread ratio a head conditioning ONLY on WHICH OPPONENT its own `V` reveals would exhibit — `sum_o q(o|V)*p_o` — and it is read BESIDE `cond.spread_ratio.<window>` to split that row into an OPPONENT-IDENTITY part and a remainder. 🚨 IT IS NOT AN UPPER BOUND ON THAT ROW AND `V` ROUTINELY EXCEEDS IT: measured on the hp800 lambda09/ctrl10M pair, V reads 0.647 / 0.704 at turns 11-24 against this row's 0.214 / 0.187. A per-state conditional mean is an ATTENUATING transform, so the cell-mean spread of `E[p_o | V]` is smaller than the cell-mean spread of `V` itself; the excess is between-opponent spread that rides on BOARD STATE correlated with the opponent rather than on opponent identity. A delta between two sides is a difference in how much opponent IDENTITY each side's output carries; it is reported, and it is never DETECTED) |
| WITHIN-own-team Murphy resolution of V (cells >= MIN_TEAM_BATTLES battles, battle-weighted over teams) | `all states, <=2 per battle` | MATCHED 797/334/9 | +0.0512 | +0.0532 | **-0.0020** | [-0.0070, +0.0009] | 2000 | **WITHIN FLOOR** (|delta| <= floor 0.0044) |
| the same resolution WITHIN team-STRENGTH strata (quantiles of the team's LOO win rate) | `all states, <=2 per battle` | MATCHED 797/334/9 | +0.0293 | +0.0305 | **-0.0012** | [-0.0047, +0.0022] | 2000 | **WITHIN FLOOR** (|delta| <= floor 0.0021) |
| BETWEEN-team spread ratio sd(mean V) / sd(team win rate), noise-corrected | `turn 1-3` | MATCHED 797/334/9 | +1.0994 | +1.2503 | **-0.1509** | [-0.1654, +0.0437] | 2000 | **WITHIN FLOOR** (|delta| <= floor 0.4324) |
| the same ratio UNCORRECTED and unclamped | `turn 1-3` | MATCHED 797/334/9 | +0.6130 | +0.6746 | **-0.0615** | [-0.0982, -0.0040] | 2000 | **WITHIN FLOOR** (|delta| <= floor 0.2507) |
| own-team leave-one-battle-out win-rate R^2 of V | `turn >= 25` | MATCHED 797/334/9 | +0.0058 | +0.0043 | **+0.0015** | [-0.0051, +0.0080] | 2000 | **NOT DETECTED** (CI covers zero) |
| own-team R^2 at turn 1 MINUS own-team R^2 late | `turn 1 - turn >= 25` | MATCHED 797/334/9 | +0.0531 | +0.0419 | **+0.0112** | [-0.0036, +0.0260] | 2000 | **PROVISIONAL — no floor; never DETECTED** (NO FLOOR EXISTS FOR THIS ROW AND THE CONTROLS CANNOT SUPPLY ONE — every control reads own-team R^2 ~= 0 at turn 1, so there is nothing for it to FALL from and the two-draw replicate floor cannot be formed. The first replicate arm supplies it. A LARGE move either way is informative; a small one is not; and this row is never DETECTED) |
| calibration SLOPE — weighted logistic regression of the outcome on logit(V) (1 = correctly dispersed, >1 = SHRUNK, <1 = over-dispersed) | `all states, <=2 per battle` | as traced | +1.2211 | +1.0695 | **+0.1516** | [+0.0731, +0.2303] | 2000 | **NOT DETECTED** (CI does not clear the floor 0.1085) |
| calibration-in-the-large — the INTERCEPT of that regression (0 = calibrated) | `all states, <=2 per battle` | as traced | +0.2182 | +0.3471 | **-0.1289** | [-0.2450, -0.0066] | 2000 | **WITHIN FLOOR** (|delta| <= floor 0.2642) |
| calibration SLOPE — weighted logistic regression of the outcome on logit(V) | `turn 1-3, <=2 per battle` | as traced | +0.8341 | +0.7324 | **+0.1017** | [-0.0319, +0.2383] | 2000 | **WITHIN FLOOR** (|delta| <= floor 0.1802) |
| calibration-in-the-large — the INTERCEPT of that regression | `turn 1-3, <=2 per battle` | as traced | +0.9064 | +0.9827 | **-0.0763** | [-0.2322, +0.0918] | 2000 | **WITHIN FLOOR** (|delta| <= floor 0.3514) |
| calibration SLOPE with a free intercept PER own-team STRENGTH STRATUM — the dispersion reading INSIDE a stratum rather than across teams | `all states, <=2 per battle, stratum fixed effects` | MATCHED 797/334/9 | +1.1580 | +1.0008 | **+0.1571** | [+0.0741, +0.2378] | 2000 | **NOT DETECTED** (CI does not clear the floor 0.1363) |
| calibration SLOPE on the COMMON SUPPORT — both sides restricted to the intersection of their central 95% of V, so the lever arm sd(logit V) cannot differ between them | `all states, <=2 per battle, common V window` | as traced | +1.2426 | +1.0619 | **+0.1806** | [+0.0921, +0.2662] | 2000 | **NOT DETECTED** (CI does not clear the floor 0.1195) |
| calibration-in-the-large on the COMMON SUPPORT | `all states, <=2 per battle, common V window` | as traced | +0.1889 | +0.3591 | **-0.1702** | [-0.2940, -0.0344] | 2000 | **WITHIN FLOOR** (|delta| <= floor 0.2778) |
...
|---|---|---|---|---|
| arm `ai_v12_13_ladder_tdaux` | `t1_3` | +0.1241 | +0.0203 | 6.121 |
| arm `ai_v12_13_ladder_tdaux` | `t4_10` | +0.4659 | +0.1314 | 3.546 |
| arm `ai_v12_13_ladder_tdaux` | `t11_24` | +0.6579 | +0.1709 | 3.849 |
...
|---|---|---|---|---|---|---|---|
| arm · team | 600 | 602 | 9555 | 19110 | 13.0 | 26.0 | 4–94 |
| arm · stratum | 5 | 602 | 9555 | 19110 | 1909.0 | 3818.0 | 1897–1925 |
| arm · between-team spread | 600 | 602 | 9555 | — | 13.0 | — | — |
...
|---|---|---|---|---|---|---|
| WITHIN-own-team Murphy resolution of V (cells >= MIN_TEAM_BATTLES battles, battle-weighted over teams) · `all states, <=2 per battle` | MATCHED 797/334/9 | +0.0512 | +0.0532 | **-0.0020** | [-0.0070, +0.0009] | **WITHIN FLOOR** (|delta| <= floor 0.0044) |
| the same resolution WITHIN team-STRENGTH strata (quantiles of the team's LOO win rate) · `all states, <=2 per battle` | MATCHED 797/334/9 | +0.0293 | +0.0305 | **-0.0012** | [-0.0047, +0.0022] | **WITHIN FLOOR** (|delta| <= floor 0.0021) |
| BETWEEN-team spread ratio sd(mean V) / sd(team win rate), noise-corrected · `turn 1-3` | MATCHED 797/334/9 | +1.0994 | +1.2503 | **-0.1509** | [-0.1654, +0.0437] | **WITHIN FLOOR** (|delta| <= floor 0.4324) |
| own-team leave-one-battle-out win-rate R^2 of V · `turn 1` | MATCHED 797/334/9 | +0.0589 | +0.0466 | **+0.0123** | [-0.0008, +0.0262] | **NOT DETECTED** (CI covers zero) |
| own-team leave-one-battle-out win-rate R^2 of V · `turn >= 25` | MATCHED 797/334/9 | +0.0058 | +0.0043 | **+0.0015** | [-0.0051, +0.0080] | **NOT DETECTED** (CI covers zero) |
| own-team R^2 at turn 1 MINUS own-team R^2 late · `turn 1 - turn >= 25` | MATCHED 797/334/9 | +0.0531 | +0.0419 | **+0.0112** | [-0.0036, +0.0260] | **PROVISIONAL — no floor; never DETECTED** (NO FLOOR EXISTS FOR THIS ROW AND THE CONTROLS CANNOT SUPPLY ONE — every control reads own-team R^2 ~= 0 at turn 1, so there is nothing for it to FALL from and the two-draw replicate floor cannot be formed. The first replicate arm supplies it. A LARGE move either way is informative; a small one is not; and this row is never DETECTED) |
...
|---|---|---|---|---|---|---|---|---|
| arm · all | `all` | 19122 | 9561 | 0.8266 | **0.1822** | **1.5289** | [0.257, 0.994] | 0.0000 |
| arm · t1_3 | `t1_3` | 19122 | 9561 | 0.7714 | **0.0934** | **0.5275** | [0.538, 0.909] | 0.0000 |
| arm · common support | `common support` | 18166 | 9460 | 0.8404 | **0.1457** | **1.2922** | [0.433, 0.992] | 0.0000 |
...
| control · common support | `common support` | 17445 | 9336 | 0.8362 | **0.1580** | **1.3798** | [0.386, 0.992] | 0.0000 |

The COMMON-SUPPORT window is `V ∈ [0.2575, 0.9942]` — the intersection of the two sides' central 95% of `V`. Both sides are re-fitted inside it, so the lever arm cannot differ between them and a surviving slope difference is not the support.

| row | frame | arm | control | **Δ** | 95% CI | replicate floor | verdict |
|---|---|---|---|---|---|---|---|
| calibration SLOPE · `all states, <=2 per battle` | as traced | +1.2211 | +1.0695 | **+0.1516** | [+0.0731, +0.2303] | 0.1085 | **NOT DETECTED** (CI does not clear the floor 0.1085) |
| calibration-in-the-large · `all states, <=2 per battle` | as traced | +0.2182 | +0.3471 | **-0.1289** | [-0.2450, -0.0066] | 0.2642 | **WITHIN FLOOR** (|delta| <= floor 0.2642) |
| calibration SLOPE · `turn 1-3, <=2 per battle` | as traced | +0.8341 | +0.7324 | **+0.1017** | [-0.0319, +0.2383] | 0.1802 | **WITHIN FLOOR** (|delta| <= floor 0.1802) |
| calibration-in-the-large · `turn 1-3, <=2 per battle` | as traced | +0.9064 | +0.9827 | **-0.0763** | [-0.2322, +0.0918] | 0.3514 | **WITHIN FLOOR** (|delta| <= floor 0.3514) |
| calibration SLOPE with a free intercept PER own-team STRENGTH STRATUM · `all states, <=2 per battle, stratum fixed effects` | MATCHED 797/334/9 | +1.1580 | +1.0008 | **+0.1571** | [+0.0741, +0.2378] | 0.1363 | **NOT DETECTED** (CI does not clear the floor 0.1363) |
| calibration SLOPE on the COMMON SUPPORT · `all states, <=2 per battle, common V window` | as traced | +1.2426 | +1.0619 | **+0.1806** | [+0.0921, +0.2662] | 0.1195 | **NOT DETECTED** (CI does not clear the floor 0.1195) |
| calibration-in-the-large on the COMMON SUPPORT · `all states, <=2 per battle, common V window` | as traced | +0.1889 | +0.3591 | **-0.1702** | [-0.2940, -0.0344] | 0.2778 | **WITHIN FLOOR** (|delta| <= floor 0.2778) |
...
| row | frame | arm | control | **Δ** | 95% CI | verdict |
|---|---|---|---|---|---|---|
| `cond.spread_ratio_raw.t1_3` | MATCHED · battle 797/334/9 (9523 battles, 9517 decoder battles, 21 seeds) | +0.1262 [+0.1453, +0.1489] | +0.1476 | **-0.0214** | [-0.0481, +0.0037] | **WITHIN FLOOR** (|delta| <= floor 0.0535) |
| `cond.spread_ratio_raw.t1_3` | MATCHED · decoder 897/376/9 (9557 battles, 9551 decoder battles, 21 seeds) | +0.1262 [+0.1477, +0.1477] | +0.1477 | **-0.0215** | [-0.0505, +0.0050] | **WITHIN FLOOR** (|delta| <= floor 0.0535) |
...
| `cond.opp_class_auc.t1_3` | MATCHED · battle 797/334/9 (9523 battles, 9517 decoder battles, 21 seeds) | +0.5848 [+0.5774, +0.5800] | +0.5790 | **+0.0058** | [-0.0106, +0.0221] | **WITHIN FLOOR** (|delta| <= floor 0.0153) |
| `cond.opp_class_auc.t1_3` | MATCHED · decoder 897/376/9 (9557 battles, 9551 decoder battles, 21 seeds) | +0.5848 [+0.5796, +0.5796] | +0.5796 | **+0.0052** | [-0.0122, +0.0212] | **WITHIN FLOOR** (|delta| <= floor 0.0153) |
| `cond.opp_class_auc.t1_3` | UNMATCHED (as traced) | +0.5848 | +0.5796 | **+0.0052** | [-0.0122, +0.0212] | *no label — not a reading* |
| `cond.opp_class_auc.t4_10` | MATCHED · battle 797/334/9 (9523 battles, 9517 decoder battles, 21 seeds) | +0.7213 [+0.7091, +0.7116] | +0.7103 | **+0.0111** | [-0.0033, +0.0244] | **WITHIN FLOOR** (|delta| <= floor 0.0219) |
| `cond.opp_class_auc.t4_10` | MATCHED · decoder 897/376/9 (9557 battles, 9551 decoder battles, 21 seeds) | +0.7213 [+0.7086, +0.7086] | +0.7086 | **+0.0127** | [-0.0021, +0.0270] | **WITHIN FLOOR** (|delta| <= floor 0.0219) |
| `cond.opp_class_auc.t4_10` | UNMATCHED (as traced) | +0.7213 | +0.7086 | **+0.0127** | [-0.0021, +0.0270] | *no label — not a reading* |
| `cond.spread_ratio_optimal.t1_3` | MATCHED · battle 797/334/9 (9523 battles, 9517 decoder battles, 21 seeds) | +0.0203 [+0.0171, +0.0181] | +0.0178 | **+0.0025** | [-0.0009, +0.0060] | **PROVISIONAL — no floor; never DETECTED** (a DESCRIPTIVE DECOMPOSITION TERM, never a pass/fail bar and never a treatment effect. It is the between-opponent spread ratio a head conditioning ONLY on WHICH OPPONENT its own `V` reveals would exhibit — `sum_o q(o|V)*p_o` — and it is read BESIDE `cond.spread_ratio.<window>` to split that row into an OPPONENT-IDENTITY part and a remainder. 🚨 IT IS NOT AN UPPER BOUND ON THAT ROW AND `V` ROUTINELY EXCEEDS IT: measured on the hp800 lambda09/ctrl10M pair, V reads 0.647 / 0.704 at turns 11-24 against this row's 0.214 / 0.187. A per-state conditional mean is an ATTENUATING transform, so the cell-mean spread of `E[p_o | V]` is smaller than the cell-mean spread of `V` itself; the excess is between-opponent spread that rides on BOARD STATE correlated with the opponent rather than on opponent identity. A delta between two sides is a difference in how much opponent IDENTITY each side's output carries; it is reported, and it is never DETECTED) |
| `cond.spread_ratio_optimal.t1_3` | MATCHED · decoder 897/376/9 (9557 battles, 9551 decoder battles, 21 seeds) | +0.0203 [+0.0181, +0.0181] | +0.0181 | **+0.0022** | [-0.0015, +0.0057] | **PROVISIONAL — no floor; never DETECTED** (a DESCRIPTIVE DECOMPOSITION TERM, never a pass/fail bar and never a treatment effect. It is the between-opponent spread ratio a head conditioning ONLY on WHICH OPPONENT its own `V` reveals would exhibit — `sum_o q(o|V)*p_o` — and it is read BESIDE `cond.spread_ratio.<window>` to split that row into an OPPONENT-IDENTITY part and a remainder. 🚨 IT IS NOT AN UPPER BOUND ON THAT ROW AND `V` ROUTINELY EXCEEDS IT: measured on the hp800 lambda09/ctrl10M pair, V reads 0.647 / 0.704 at turns 11-24 against this row's 0.214 / 0.187. A per-state conditional mean is an ATTENUATING transform, so the cell-mean spread of `E[p_o | V]` is smaller than the cell-mean spread of `V` itself; the excess is between-opponent spread that rides on BOARD STATE correlated with the opponent rather than on opponent identity. A delta between two sides is a difference in how much opponent IDENTITY each side's output carries; it is reported, and it is never DETECTED) |
| `cond.spread_ratio_optimal.t1_3` | UNMATCHED (as traced) | +0.0203 | +0.0181 | **+0.0022** | [-0.0015, +0.0057] | *no label — not a reading* |
| `cond.spread_ratio_optimal.t4_10` | MATCHED · battle 797/334/9 (9523 battles, 9517 decoder battles, 21 seeds) | +0.1314 [+0.1247, +0.1276] | +0.1262 | **+0.0052** | [-0.0107, +0.0215] | **PROVISIONAL — no floor; never DETECTED** (a DESCRIPTIVE DECOMPOSITION TERM, never a pass/fail bar and never a treatment effect. It is the between-opponent spread ratio a head conditioning ONLY on WHICH OPPONENT its own `V` reveals would exhibit — `sum_o q(o|V)*p_o` — and it is read BESIDE `cond.spread_ratio.<window>` to split that row into an OPPONENT-IDENTITY part and a remainder. 🚨 IT IS NOT AN UPPER BOUND ON THAT ROW AND `V` ROUTINELY EXCEEDS IT: measured on the hp800 lambda09/ctrl10M pair, V reads 0.647 / 0.704 at turns 11-24 against this row's 0.214 / 0.187. A per-state conditional mean is an ATTENUATING transform, so the cell-mean spread of `E[p_o | V]` is smaller than the cell-mean spread of `V` itself; the excess is between-opponent spread that rides on BOARD STATE correlated with the opponent rather than on opponent identity. A delta between two sides is a difference in how much opponent IDENTITY each side's output carries; it is reported, and it is never DETECTED) |
| `cond.spread_ratio_optimal.t4_10` | MATCHED · decoder 897/376/9 (9557 battles, 9551 decoder battles, 21 seeds) | +0.1314 [+0.1264, +0.1264] | +0.1264 | **+0.0050** | [-0.0113, +0.0219] | **PROVISIONAL — no floor; never DETECTED** (a DESCRIPTIVE DECOMPOSITION TERM, never a pass/fail bar and never a treatment effect. It is the between-opponent spread ratio a head conditioning ONLY on WHICH OPPONENT its own `V` reveals would exhibit — `sum_o q(o|V)*p_o` — and it is read BESIDE `cond.spread_ratio.<window>` to split that row into an OPPONENT-IDENTITY part and a remainder. 🚨 IT IS NOT AN UPPER BOUND ON THAT ROW AND `V` ROUTINELY EXCEEDS IT: measured on the hp800 lambda09/ctrl10M pair, V reads 0.647 / 0.704 at turns 11-24 against this row's 0.214 / 0.187. A per-state conditional mean is an ATTENUATING transform, so the cell-mean spread of `E[p_o | V]` is smaller than the cell-mean spread of `V` itself; the excess is between-opponent spread that rides on BOARD STATE correlated with the opponent rather than on opponent identity. A delta between two sides is a difference in how much opponent IDENTITY each side's output carries; it is reported, and it is never DETECTED) |
| `cond.spread_ratio_optimal.t4_10` | UNMATCHED (as traced) | +0.1314 | +0.1264 | **+0.0050** | [-0.0113, +0.0219] | *no label — not a reading* |
| `cond.spread_ratio_optimal.t11_24` | MATCHED · battle 797/334/9 (9523 battles, 9517 decoder battles, 21 seeds) | +0.1709 [+0.1855, +0.1877] | +0.1867 | **-0.0158** | [-0.0383, +0.0072] | **PROVISIONAL — no floor; never DETECTED** (a DESCRIPTIVE DECOMPOSITION TERM, never a pass/fail bar and never a treatment effect. It is the between-opponent spread ratio a head conditioning ONLY on WHICH OPPONENT its own `V` reveals would exhibit — `sum_o q(o|V)*p_o` — and it is read BESIDE `cond.spread_ratio.<window>` to split that row into an OPPONENT-IDENTITY part and a remainder. 🚨 IT IS NOT AN UPPER BOUND ON THAT ROW AND `V` ROUTINELY EXCEEDS IT: measured on the hp800 lambda09/ctrl10M pair, V reads 0.647 / 0.704 at turns 11-24 against this row's 0.214 / 0.187. A per-state conditional mean is an ATTENUATING transform, so the cell-mean spread of `E[p_o | V]` is smaller than the cell-mean spread of `V` itself; the excess is between-opponent spread that rides on BOARD STATE correlated with the opponent rather than on opponent identity. A delta between two sides is a difference in how much opponent IDENTITY each side's output carries; it is reported, and it is never DETECTED) |
| `cond.spread_ratio_optimal.t11_24` | MATCHED · decoder 897/376/9 (9557 battles, 9551 decoder battles, 21 seeds) | +0.1709 [+0.1871, +0.1871] | +0.1871 | **-0.0161** | [-0.0392, +0.0070] | **PROVISIONAL — no floor; never DETECTED** (a DESCRIPTIVE DECOMPOSITION TERM, never a pass/fail bar and never a treatment effect. It is the between-opponent spread ratio a head conditioning ONLY on WHICH OPPONENT its own `V` reveals would exhibit — `sum_o q(o|V)*p_o` — and it is read BESIDE `cond.spread_ratio.<window>` to split that row into an OPPONENT-IDENTITY part and a remainder. 🚨 IT IS NOT AN UPPER BOUND ON THAT ROW AND `V` ROUTINELY EXCEEDS IT: measured on the hp800 lambda09/ctrl10M pair, V reads 0.647 / 0.704 at turns 11-24 against this row's 0.214 / 0.187. A per-state conditional mean is an ATTENUATING transform, so the cell-mean spread of `E[p_o | V]` is smaller than the cell-mean spread of `V` itself; the excess is between-opponent spread that rides on BOARD STATE correlated with the opponent rather than on opponent identity. A delta between two sides is a difference in how much opponent IDENTITY each side's output carries; it is reported, and it is never DETECTED) |
| `cond.spread_ratio_optimal.t11_24` | UNMATCHED (as traced) | +0.1709 | +0.1871 | **-0.0161** | [-0.0392, +0.0070] | *no label — not a reading* |
| `cond.within_team_resolution.all` | MATCHED · battle 797/334/9 (9523 battles, 9517 decoder battles, 21 seeds) | +0.0512 [+0.0520, +0.0538] | +0.0532 | **-0.0020** | [-0.0070, +0.0009] | **WITHIN FLOOR** (|delta| <= floor 0.0044) |
| `cond.within_team_resolution.all` | MATCHED · decoder 897/376/9 (9557 battles, 9551 decoder battles, 21 seeds) | +0.0512 [+0.0531, +0.0531] | +0.0531 | **-0.0019** | [-0.0074, +0.0010] | **WITHIN FLOOR** (|delta| <= floor 0.0044) |
| `cond.within_team_resolution.all` | UNMATCHED (as traced) | +0.0512 | +0.0531 | **-0.0019** | [-0.0074, +0.0010] | *no label — not a reading* |
| `cond.within_stratum_resolution.all` | MATCHED · battle 797/334/9 (9523 battles, 9517 decoder battles, 21 seeds) | +0.0293 [+0.0302, +0.0309] | +0.0305 | **-0.0012** | [-0.0047, +0.0022] | **WITHIN FLOOR** (|delta| <= floor 0.0021) |
| `cond.within_stratum_resolution.all` | MATCHED · decoder 897/376/9 (9557 battles, 9551 decoder battles, 21 seeds) | +0.0293 [+0.0308, +0.0308] | +0.0308 | **-0.0015** | [-0.0051, +0.0020] | **WITHIN FLOOR** (|delta| <= floor 0.0021) |
| `cond.within_stratum_resolution.all` | UNMATCHED (as traced) | +0.0293 | +0.0308 | **-0.0015** | [-0.0051, +0.0020] | *no label — not a reading* |
| `cond.team_spread_ratio.t1_3` | MATCHED · battle 797/334/9 (9523 battles, 9517 decoder battles, 21 seeds) | +1.0994 [+1.2128, +1.3066] | +1.2503 | **-0.1509** | [-0.1654, +0.0437] | **WITHIN FLOOR** (|delta| <= floor 0.4324) |
| `cond.team_spread_ratio.t1_3` | MATCHED · decoder 897/376/9 (9557 battles, 9551 decoder battles, 21 seeds) | +1.0994 [+1.2523, +1.2523] | +1.2523 | **-0.1529** | [-0.1726, +0.0392] | **WITHIN FLOOR** (|delta| <= floor 0.4324) |
| `cond.team_spread_ratio.t1_3` | UNMATCHED (as traced) | +1.0994 | +1.2523 | **-0.1529** | [-0.1726, +0.0392] | *no label — not a reading* |
| `cond.team_spread_ratio_raw.t1_3` | MATCHED · battle 797/334/9 (9523 battles, 9517 decoder battles, 21 seeds) | +0.6130 [+0.6700, +0.6824] | +0.6746 | **-0.0615** | [-0.0982, -0.0040] | **WITHIN FLOOR** (|delta| <= floor 0.2507) |
| `cond.team_spread_ratio_raw.t1_3` | MATCHED · decoder 897/376/9 (9557 battles, 9551 decoder battles, 21 seeds) | +0.6130 [+0.6791, +0.6791] | +0.6791 | **-0.0660** | [-0.1035, -0.0029] | **WITHIN FLOOR** (|delta| <= floor 0.2507) |
| `cond.team_spread_ratio_raw.t1_3` | UNMATCHED (as traced) | +0.6130 | +0.6791 | **-0.0660** | [-0.1035, -0.0029] | *no label — not a reading* |
| `cond.own_team_r2.late` | MATCHED · battle 797/334/9 (9523 battles, 9517 decoder battles, 21 seeds) | +0.0058 [+0.0027, +0.0052] | +0.0043 | **+0.0015** | [-0.0051, +0.0080] | **NOT DETECTED** (CI covers zero) |
| `cond.own_team_r2.late` | MATCHED · decoder 897/376/9 (9557 battles, 9551 decoder battles, 21 seeds) | +0.0058 [+0.0050, +0.0050] | +0.0050 | **+0.0008** | [-0.0056, +0.0070] | **WITHIN FLOOR** (|delta| <= floor 0.0012) |
| `cond.own_team_r2.late` | UNMATCHED (as traced) | +0.0058 | +0.0050 | **+0.0008** | [-0.0056, +0.0070] | *no label — not a reading* |
| `cond.own_team_r2.t1_minus_late` | MATCHED · battle 797/334/9 (9523 battles, 9517 decoder battles, 21 seeds) | +0.0531 [+0.0394, +0.0449] | +0.0419 | **+0.0112** | [-0.0036, +0.0260] | **PROVISIONAL — no floor; never DETECTED** (NO FLOOR EXISTS FOR THIS ROW AND THE CONTROLS CANNOT SUPPLY ONE — every control reads own-team R^2 ~= 0 at turn 1, so there is nothing for it to FALL from and the two-draw replicate floor cannot be formed. The first replicate arm supplies it. A LARGE move either way is informative; a small one is not; and this row is never DETECTED) |
| `cond.own_team_r2.t1_minus_late` | MATCHED · decoder 897/376/9 (9557 battles, 9551 decoder battles, 21 seeds) | +0.0531 [+0.0424, +0.0424] | +0.0424 | **+0.0106** | [-0.0036, +0.0255] | **PROVISIONAL — no floor; never DETECTED** (NO FLOOR EXISTS FOR THIS ROW AND THE CONTROLS CANNOT SUPPLY ONE — every control reads own-team R^2 ~= 0 at turn 1, so there is nothing for it to FALL from and the two-draw replicate floor cannot be formed. The first replicate arm supplies it. A LARGE move either way is informative; a small one is not; and this row is never DETECTED) |
| `cond.own_team_r2.t1_minus_late` | UNMATCHED (as traced) | +0.0531 | +0.0424 | **+0.0106** | [-0.0036, +0.0255] | *no label — not a reading* |
| `cond.calibration_slope.within_stratum` | MATCHED · battle 797/334/9 (9523 battles, 9517 decoder battles, 21 seeds) | +1.1580 [+0.9898, +1.0070] | +1.0008 | **+0.1571** | [+0.0741, +0.2378] | **NOT DETECTED** (CI does not clear the floor 0.1363) |
| `cond.calibration_slope.within_stratum` | MATCHED · decoder 897/376/9 (9557 battles, 9551 decoder battles, 21 seeds) | +1.1580 [+1.0200, +1.0200] | +1.0200 | **+0.1380** | [+0.0594, +0.2173] | **NOT DETECTED** (CI does not clear the floor 0.1363) |
| `cond.calibration_slope.within_stratum` | UNMATCHED (as traced) | +1.1580 | +1.0200 | **+0.1380** | [+0.0594, +0.2173] | *no label — not a reading* |
...
| between-opponent spread ratio sd(V)/sd(outcome), noise-corrected · `turn 4-10` | +0.4659 | [+0.4248, +0.5143] | +0.5080 | [+0.4626, +0.5569] |
| between-opponent spread ratio sd(V)/sd(outcome), noise-corrected · `turn 11-24` | +0.6579 | [+0.5974, +0.7301] | +0.7039 | [+0.6391, +0.7725] |
...
| opponent-CLASS (pool vs bot) AUC of V · `turn 1-3` | +0.5848 | [+0.5727, +0.5964] | +0.5796 | [+0.5675, +0.5917] |
| opponent-CLASS (pool vs bot) AUC of V · `turn 4-10` | +0.7213 | [+0.7114, +0.7305] | +0.7086 | [+0.6987, +0.7188] |
| OPPONENT-DECODABLE between-opponent spread ratio — sum_o q(o|V)*p_o, noise-corrected (NOT a bound on the row above) · `turn 1-3` | +0.0203 | [+0.0181, +0.0231] | +0.0181 | [+0.0159, +0.0207] |
| OPPONENT-DECODABLE between-opponent spread ratio — sum_o q(o|V)*p_o, noise-corrected (NOT a bound on the row above) · `turn 4-10` | +0.1314 | [+0.1202, +0.1445] | +0.1264 | [+0.1157, +0.1380] |
| OPPONENT-DECODABLE between-opponent spread ratio — sum_o q(o|V)*p_o, noise-corrected (NOT a bound on the row above) · `turn 11-24` | +0.1709 | [+0.1566, +0.1873] | +0.1871 | [+0.1713, +0.2033] |
| WITHIN-own-team Murphy resolution of V (cells >= MIN_TEAM_BATTLES battles, battle-weighted over teams) · `all states, <=2 per battle` | +0.0512 | [+0.0511, +0.0569] | +0.0531 | [+0.0541, +0.0601] |
| the same resolution WITHIN team-STRENGTH strata (quantiles of the team's LOO win rate) · `all states, <=2 per battle` | +0.0293 | [+0.0273, +0.0321] | +0.0308 | [+0.0285, +0.0338] |
| BETWEEN-team spread ratio sd(mean V) / sd(team win rate), noise-corrected · `turn 1-3` | +1.0994 | [+0.5537, +0.6974] | +1.2523 | [+0.6046, +0.7674] |
| the same ratio UNCORRECTED and unclamped · `turn 1-3` | +0.6130 | [+0.4498, +0.5143] | +0.6791 | [+0.4983, +0.5740] |
| own-team leave-one-battle-out win-rate R^2 of V · `turn >= 25` | +0.0058 | [+0.0009, +0.0101] | +0.0050 | [+0.0006, +0.0091] |
| own-team R^2 at turn 1 MINUS own-team R^2 late · `turn 1 - turn >= 25` | +0.0531 | [+0.0430, +0.0638] | +0.0424 | [+0.0327, +0.0523] |
| calibration SLOPE — weighted logistic regression of the outcome on logit(V) (1 = correctly dispersed, >1 = SHRUNK, <1 = over-dispersed) · `all states, <=2 per battle` | +1.2211 | [+1.1638, +1.2817] | +1.0695 | [+1.0223, +1.1229] |
| calibration-in-the-large — the INTERCEPT of that regression (0 = calibrated) · `all states, <=2 per battle` | +0.2182 | [+0.1337, +0.3075] | +0.3471 | [+0.2648, +0.4224] |
| calibration SLOPE — weighted logistic regression of the outcome on logit(V) · `turn 1-3, <=2 per battle` | +0.8341 | [+0.7325, +0.9348] | +0.7324 | [+0.6476, +0.8189] |
| calibration-in-the-large — the INTERCEPT of that regression · `turn 1-3, <=2 per battle` | +0.9064 | [+0.7826, +1.0384] | +0.9827 | [+0.8779, +1.0856] |
| calibration SLOPE with a free intercept PER own-team STRENGTH STRATUM — the dispersion reading INSIDE a stratum rather than across teams · `all states, <=2 per battle, stratum fixed effects` | +1.1580 | [+1.0994, +1.2199] | +1.0200 | [+0.9727, +1.0739] |
| calibration SLOPE on the COMMON SUPPORT — both sides restricted to the intersection of their central 95% of V, so the lever arm sd(logit V) cannot differ between them · `all states, <=2 per battle, common V window` | +1.2426 | [+1.1791, +1.3096] | +1.0619 | [+1.0058, +1.1240] |
| calibration-in-the-large on the COMMON SUPPORT · `all states, <=2 per battle, common V window` | +0.1889 | [+0.0977, +0.2812] | +0.3591 | [+0.2675, +0.4460] |
...
| this report | `/home/goodlad/.claude/jobs/9ab51de6/tmp/hp_eval/hp800/reads/ai_v12_13_ladder_tdaux_vs_ctrl10M_v6/critic_read.md` |
| machine-readable | `/home/goodlad/.claude/jobs/9ab51de6/tmp/hp_eval/hp800/reads/ai_v12_13_ladder_tdaux_vs_ctrl10M_v6/critic_read.json` |
