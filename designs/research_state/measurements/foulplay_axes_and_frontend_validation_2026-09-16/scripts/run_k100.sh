#!/usr/bin/env bash
set -u
cd /home/goodlad/dev/gen3ai
export PYTHONPATH="${PYTHONPATH:-}:/home/goodlad/dev/gen3ai/src"
TMP=/home/goodlad/.claude/jobs/9ab51de6/tmp/fp_axes
PY=/home/goodlad/miniconda3/envs/gen3ai_stable/bin/python3
echo "=== CELL k100 START $(date -Is) load: $(cut -d' ' -f1-3 /proc/loadavg) head: $(git rev-parse --short HEAD)" | tee $TMP/cells/k100.meta
rm -rf $TMP/cells/k100
env OMP_NUM_THREADS=1 MKL_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 NUMEXPR_NUM_THREADS=1 \
    CUDA_VISIBLE_DEVICES="" GEN3AI_ANCHORS_CONFIG=$TMP/cfg/anchors_noknow.json \
  nice -n 10 $PY -m main.anchors \
    --model models/ai_v12_02_winprob_critic/final_model.zip --opponent foulplay \
    --regime greedy --teamset home --games 80 --search-time-ms 100 --search-parallelism 1 \
    --team-seed 916001 --port 9745 --username AxK100 --peer-username FoulPlayK1 \
    --progress-timeout 1800 --first-game-timeout 1800 \
    --out $TMP/cells/k100 > $TMP/cells/k100.log 2>&1
echo "=== CELL k100 END $(date -Is) load: $(cut -d' ' -f1-3 /proc/loadavg) rc=$?" | tee -a $TMP/cells/k100.meta
grep -E '^  status|^  WIN RATE|^  search ' $TMP/cells/k100.log | tee -a $TMP/cells/k100.meta
echo "K100 DONE"
