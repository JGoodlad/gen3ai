"""Score THIS directory's mirror cells (Part A's three unread arms + their contemporaneous
anchor, and Part C's refit head vs its control).

Adapted from `search_dividend_winprob_heads_2026-09-11/report2.py` with ONE deliberate change:
**the fixed L1 bar is GONE.** UNDERSTANDING rule 23 convicted separation-of-raced as a WIDTH
meter — the same checkpoint, cell, flags and battles read L1 = 0.058 / 0.128 / 0.365 at realized
K = 3.6 / 5.1 / 8.9 — so the 18.1 % bar is withdrawn and L1 is printed only BESIDE its realized K,
to be read against a contemporaneous control or through `l1_width_matched.py`. Printing a verdict
against a fixed bar here would reproduce exactly the false positive that rule exists to prevent.
...
def l1_verdict(sep_rate):
    """There is NO fixed L1 bar any more (rule 23). L1 is a width meter; this returns the
    instruction rather than a verdict, so a reader cannot take one from this column."""
    return "-" if sep_rate is None else "read vs contemporaneous control @ matched K"
...
def main() -> None:
    ...
    heads = ["anchor", "rollout", "ent05", "vf025", "refit", "orig"]
    ...
    print()
    print("L1/L2 LEAF-QUALITY METER — L1 has NO fixed bar (rule 23: it is a WIDTH meter; its "
          "realized K is printed beside it); L2 bar = paired CI lower bound > 0.50")
    print(f"{'cell/head':26} {'L1 sep/raced':>12} {'K worlds':>9} {'L2 paired':>10} "
          f"{'L2 CI':>18} {'L2 verdict':22}")
    for k, r in out["cells"].items():
        d = r["defensive"] or {}
        if not d:
            continue
        sep = d.get("separation_rate")
        ci = ("-" if not r["paired_ci95"] else
              f"[{r['paired_ci95'][0]:.4f},{r['paired_ci95'][1]:.4f}]")
        pw = "-" if r["paired_win_rate"] is None else f"{r['paired_win_rate']:.4f}"
        l2 = "CLEARS" if (r["paired_ci95"] and r["paired_ci95"][0] > 0.50) else "below bar"
        out["cells"][k]["L1_sep_of_raced"] = sep
        out["cells"][k]["L1_verdict"] = l1_verdict(sep)
        out["cells"][k]["L2_verdict"] = l2
        kk = (r.get("realized_mean") or {}).get("k_worlds")
        out["cells"][k]["L1_realized_k"] = kk
        print(f"{k:26} {str(sep):>12} {str(kk):>9} {pw:>10} {ci:>18} {l2:22}")
    print()
