...
# on the CONVERGED ENDPOINT of the fold path (+12.09M). greedy-vs-greedy, verified per decision,
# 100 games as two role-balanced half-cells. This is the +6M read's own run_away_cell.sh with the
# ref swapped and the port moved inside this job's 9400-9499 band.
#
...
OUT=/home/goodlad/.claude/jobs/9ab51de6/tmp/fold1c_read/anchors/away
mkdir -p "$OUT"
cd /home/goodlad/dev/gen3ai
TAG=cont_p12M
[ -f "$OUT/${TAG}_smallrl_greedy_away/summary.json" ] && { echo "SKIP $TAG"; exit 0; }
echo "=== $TAG greedy away $(date -Is) ==="
nice -n 15 $P -m main.anchors --model models/ai_v13_08_fold1_cont/final_model.zip \
