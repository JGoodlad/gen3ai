#!/usr/bin/env python3
"""ROW 3 — the Metamon ``SmallRL`` greedy AWAY cell at the CONVERGED ENDPOINT (+12.09M).

The cell came through the tool of record, ``python -m main.anchors``, which starts and stops its own
Showdown server (:9451 here), verifies the greedy regime PER DECISION on both sides, and writes the
Wilson interval itself. Nothing here re-derives a win rate; this script joins the four arms
(arm W, W_b, the fold at +6M and the converged endpoint), computes the Newcombe interval on each
difference, and applies the registered floor.
...
component here is run-level, and more games are the wrong lever). Saying so before the cell ran is
...
def main() -> int:
    ...
    out = {
        ...
        "cont_p12M": {"win_rate": s["win_rate"], "wins": s["wins"], "n": s["n"],
