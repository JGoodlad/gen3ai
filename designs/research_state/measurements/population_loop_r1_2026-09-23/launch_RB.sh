#!/usr/bin/env bash
# LAUNCH — RB, ai_v13_24_popr1_read_loop: the FRESH OFFENSE EXPLOITER that reads B (the LOOP).
#
# 🚨 PREPARED BUT NEVER EXECUTED by the session that wrote it. For the TRAINING RUN session,
#    VERBATIM. It takes the GPU. It CANNOT launch until ai_v13_22_popr1_loop has FINISHED at
...
# models/ai_v13_22_popr1_loop/final_model.zip, --steps 103158272 -> 111219200 (a TOTAL: the target's
...
# pin 6eb9c776. gap(RB) = its pooled greedy vs-target rate - 0.5, read by main.best_response_gap.
# Validated on a STAND-IN target (scripts/argv_RB_STANDIN.txt, ai_v13_16_teach5_offense_dist @103,219,200).
#
# Usage:  bash designs/research_state/measurements/population_loop_r1_2026-09-23/launch_RB.sh [--dry-run]
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ARM=RB
RUN=ai_v13_24_popr1_read_loop
ARGV_FILE="$HERE/argv_RB.txt"
TARGET_ZIP=models/ai_v13_22_popr1_loop/final_model.zip
