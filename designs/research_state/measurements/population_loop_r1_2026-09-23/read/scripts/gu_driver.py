#!/usr/bin/env python3
"""G-U — the untaught-8 KILL guard of population-loop round 1, run INCREMENTALLY.

Registration: ``designs/research_state/population_loop_round1_2026-09-23.md`` §4.3. The registered
...
def aggregate(rows: Path, out: Path) -> int:
    ...
    (out / "untaught_popr1.json").write_text(json.dumps(doc, indent=1))
    (out / "untaught_popr1.md").write_text(engine.render_markdown(doc))
    ...
    guard = {
        "what": "G-U, the untaught-8 KILL guard (registration §4.3)",
        "tree": meta["tree"],
        ...
        "B_minus_C (popr1_loop - popr1_ctrl)": rule("popr1_loop", "popr1_ctrl"),
        "C_minus_G0 (popr1_ctrl - plateau_b1), descriptor": rule("popr1_ctrl", "plateau_b1"),
        "B_minus_G0 (popr1_loop - plateau_b1), descriptor": rule("popr1_loop", "plateau_b1"),
        "bootstrap": "20,000 draws, ONE shared team index set, seed 20260915; the TEAM is the unit",
    }
    bc = guard["B_minus_C (popr1_loop - popr1_ctrl)"]
