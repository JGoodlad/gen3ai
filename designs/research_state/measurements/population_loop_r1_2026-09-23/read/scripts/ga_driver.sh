#!/usr/bin/env bash
# G-A — the external-anchor KILL guard of population-loop round 1 (registration §4.3), INCREMENTAL.
#
...
set -u
TREE=/home/goodlad/dev/gen3ai-wt/pin-7c511161-popr1
...
declare -A MODEL=(
  [B_loop]=/home/goodlad/dev/gen3ai/models/ai_v13_22_popr1_loop/final_model.zip
  [C_ctrl]=/home/goodlad/dev/gen3ai/models/ai_v13_23_popr1_ctrl/final_model.zip
  [G0_plateau]=/home/goodlad/dev/gen3ai/models/ai_v13_12_plateau/final_model.zip
)
ORDER=(B_loop C_ctrl G0_plateau)
...
for SEED in 0 10 20 30 40 50; do
  for TS in away home; do
    for M in "${ORDER[@]}"; do
      ...
      nice -n 15 $P -m main.anchors --model "${MODEL[$M]}" --opponent metamon:SmallRL \
        --regime greedy --teamset "$TS" --games 100 --team-seed "$SEED" --seed-base "$SEED" \
        --device cpu --nice 15 --out "$D" > "$ROWS/$U.log" 2>&1
      rc=$?
