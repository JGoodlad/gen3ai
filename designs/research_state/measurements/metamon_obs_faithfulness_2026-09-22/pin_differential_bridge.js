...
const psPath = process.env.GEN3AI_PS_PATH ? path.resolve(process.env.GEN3AI_PS_PATH) : path.resolve(__dirname, '../../../deps/pokemon-showdown');
// 🚨 CHECK 4 ONLY. A byte-for-byte copy of src/utils/bridge/local_sim_bridge.js with ONE change:
// the Showdown checkout is taken from $GEN3AI_PS_PATH so the SAME seeded battle can be re-run
// through Metamon's bundled pin (d62d3a398) and ours (e0551883f) and the two per-side protocol
// streams compared. Keep it a copy rather than a flag on the real bridge: the real bridge's
// checkout must stay pinned, and a differential tool is not a reason to make it configurable.
