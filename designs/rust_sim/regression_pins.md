...
| TL1 the TURN-LIMIT TIE (`gen3_turn_limit_tie_v1`, the Rust Core Program's Phase-0 finding F1 — 3 of 300 random-player battles reached it) — past turn 1000 Showdown's `endTurn` adds `|message|It is turn 1000. You have hit the turn limit!` and TIES (`|` + `|tie`) BEFORE `|turn|` and the Quick Claw roll, and from turn 500 precedes `|turn|N` with a `|bigerror|` countdown. WRONG (pre-fix): the driver PANICKED on the 1,001st committed turn (`BATTLE_TURN_CAP`), so the bridge child died where Showdown ends the battle. Pinned in `tests/turn_limit_test.rs` (not `regression_test.rs`: the scenario is 1,000 turns of per-side stream) | `turn_limit_ties_instead_of_panicking` + the byte differential `turn_limit_battle_matches_showdown_byte_for_byte` / `…_from_the_raw_start_seed` (golden `tests/vectors/turn_limit_golden.txt`, `harness/gen_turn_limit_capture.js`) | STATE (tie, `turn == 1001`, the tail lines, 24 countdown lines) + BYTES (FNV-1a over both whole per-side streams + windowed lines) |
| LK1 a MOVE-LOCKED request names the LOCKED move (`gen3_locked_request_move_v1`, found by the Rust Core parity harness's procedural milestone sweep — a Registeel locked into Rollout) — Showdown's `getMoveRequestData` offers only `lockedMove` (`{"moves":[{"move":"Rollout","id":"rollout"}],"trapped":true}`) and accepts `move rollout`; WRONG (pre-fix): the request builder and the `move <name>` resolver read the TWO-TURN slot only, so every lock admitted after it (Rollout, Ice Ball, Outrage, Thrash, Petal Dance, Uproar) requested **Solar Beam** — poke-env's live player asserts on it — and, once the request was right, refused its own move by name. `MonState::locked_slot()` is now the one reader | `locked_request_test::every_move_lock_requests_the_locked_move_as_showdown_does` + `a_locked_mon_answered_BY_NAME_resolves_and_the_battle_moves_on` | BYTES (each `active` block equal to the one the pinned Showdown printed for the same format, seed, teams and choices) + STATE (no fatal, the turn advances) |
| LM1 LOCK-IN CONTINUATIONS announce `[from] lockedmove` (`gen3_lockedmove_announce_v1`, found by the LADDER-USAGE corpus's first slice-V run — a Rollout Registeel whose own-PP reading ran one ahead of its request) — `runMove`'s `getLockedMove()` branch sets `sourceEffect = lockedmove` for every turn a lock repeats the move (Outrage / Thrash / Petal Dance, Uproar, Rollout / Ice Ball), so the sim's announce is `|move|<user>|<Move>|<target>|[from] lockedmove`; WRONG (pre-fix): only Solar Beam's fire turn carried it, every other continuation was BARE and poke-env charged a PP per continuing turn. 0 pool teams carry any of the moves | `protocol_byte_fuzz_test::lockin_continuations_announce_from_lockedmove` | BYTES (the cast bare, the first continuation with the attr, for all four families) |
| RL1 ROLLOUT's lock EXPIRES on a turn that computes no damage (`gen3_rollout_lock_duration_v1`, the same probe) — the `rollout` volatile is added on the first use with `duration: 1` and refreshed to 2 only by `basePowerCallback`, so a Protect-blocked / missed / skipped turn lets it expire at that residual and the next use is FRESH (bare announce, PP paid, bp 30); WRONG (pre-fix): the port kept the lock across such a turn and never registered the volatile's residual duration handler | `protocol_byte_fuzz_test::rollout_lock_ends_on_a_turn_that_does_not_hit` (RO1 `rollout_doubles_per_execution…` now locks against a Steelix so the 5th turn's residual runs) | BYTES (bare / locked / bare) + STATE (PP 32 → 30) |
| LF1 the END-OF-LOCK confusion announces `[fatigue]` (`gen3_lockin_fatigue_v1`, found by the M6 cutover stress, `--mode ourandom --protocol`, a Dragonite Outrage) — `confusion.onStart` tags `\|-start\|<user>\|confusion\|[fatigue]` when its `sourceEffect` is `lockedmove`, which it always is inside `lockedmove.onEnd`; WRONG (pre-fix): the port emitted the BARE `-start` (poke-env's reading is unchanged — it reads only `event[2:4]` — so the obs was unaffected). Ground truth `harness/probe_lockin_fatigue.js` | `protocol_byte_fuzz_test::lock_end_confusion_announces_fatigue` + byte-fuzz fixture `76_lock_end_confusion_fatigue_tag.txt` | BYTES (Outrage / Thrash / Petal Dance: the tagged form present, the bare form absent) |
| SG3 Safeguard does NOT block a SELF-SOURCED confusion (`gen3_lockin_fatigue_v1`, found while settling LF1) — `safeguard.onTryAddVolatile` blocks confusion only when `target !== source`, and `addVolatile` defaults `source` to the holder when the ambient event has none: the holder's own lock ending (`removeVolatile`'s sourceless `End`) and its own Figy-family berry (`eatItem` sets source = the holder) confuse it and DRAW the `random(2,6)` under its own Safeguard; WRONG (pre-fix): `add_confusion` returned at any live Safeguard — no confusion, one draw fewer. Ground truth `harness/probe_lockin_fatigue.js` (raw `>start` seeds `[3,3,3,3]` / `[5,5,5,5]`) | `safeguard_does_not_block_a_self_sourced_confusion` | STATE (confused, Safeguard still up) + SEED (the probe's `seedAfter`) for both the lock end and the berry, each leg revert-verified alone |
| IM3 an IMPRISON RE-CAST while imprisoning FAILS (`gen3_imprison_recast_fails_v1`, exposed by re-recording the M6 stress's `rmuggvoke_ab_3_15` from its sim-applied choices) — the `imprison` condition has no `onRestart`, so `addVolatile` returns false after `onTryHit` passes and the move takes the draw-free `\|move\|<u>\|Imprison\|\|[still]` + `\|-fail\|<u>` form; WRONG (pre-fix): the port re-started it (a second `\|-start\|<u>\|move: Imprison`). `harness/probe_imprison.js` Q3 had claimed this row SETTLED while its script re-sent Splash, never Imprison | `imprison_recast_while_imprisoning_fails` + byte-fuzz fixture `77_imprison_recast_still_fail.txt` | BYTES + STATE (two uses, one `-start`, the `[still]` + one `-fail`, the original Imprison standing) |
| BX1 two HARD-CODED stat indices were off by one (`gen3_boost_index_fixes_v1`, found by a bounded `--mode ourandom --protocol` re-run after the M6 stress fixes: a Blissey's Defense Curl, then a Brick Break at 1.5x the sim's damage). `boosts` is in `STAT_TOKENS` order (atk 0, def 1, spa 2, spd 3, spe 4, accuracy 5, evasion 6); DEFENSE CURL wrote index 2 (SpA, announcing `\|-boost\|<u>\|spa\|1`) and RAGE's hit reaction wrote index 1 (Def, announcing `def\|1`). The sim: `{def: 1}` and `{atk: 1}` (resolved dex + a direct probe). Their pins RO2 / RG1 asserted the SAME wrong indices, so they certified the bugs; both now assert the right index, the untouched neighbour and the emitted line. 0 committed goldens executed either move | `defense_curl_raises_defense_and_sets_the_volatile` (RO2) + `rage_boosts_attack_when_the_foe_hits_inside_the_window` (RG1) + byte-fuzz fixture `78_defense_curl_boosts_def.txt` | STATE (the index + its neighbour) + BYTES (the `-boost` line), each revert-verified |
| MX1 the readers of a SELF-OVERWRITING Mimic's `lastMove` (`gen3_mimic_self_overwrite_readers_v1`, found by the M6 cutover stress, `rmugk8bpp_ab_0_5`: an Alakazam encored a Meganium into the Trick its Mimic had just copied). A successful Mimic replaces its OWN slot, but the move USED is still `mimic`, now in no slot; the sim resolves `lastMove` by ID, so ENCORE into it fails after drawing accuracy + `random(3,7)`, and a new foe's MIMIC of it fails (`failmimic`). The port stores `lastMove` as a SLOT and read the copy. Both arms now honour `last_move_was_self_overwrite` (Disable = MD1, Torment and the Choice lock already did); Mimic only for a Mimic self-overwrite — a TRANSFORM self-overwrite leaves `transform` (no `failmimic`), which the sim copies. Ground truth `harness/probe_mimic_self_overwrite_readers.js` (E1 / M1) | `a_self_overwriting_mimic_is_what_encore_and_mimic_read` + byte-fuzz fixture `79_encore_into_a_self_overwriting_mimic_fails.txt` | BYTES + STATE + SEED (the probe's `seedAfter`), each leg revert-verified alone |
| NS1 a NESTED cross-side move carries ITS USER's source scope (`gen3_core_nested_move_scope_v1`, found by the Rust Core CUTOVER stress — `pool_110_5`, 2026-09-25: Blissey snatches Swampert's Refresh with no status to cure, so the stolen Refresh FAILS on Blissey) — the sim's `useMoveInner` makes the nested user the active mon (`setActiveMove(move, pokemon, …)`), so the stolen move's `|move|…|[from] Snatch` announce and its `|-fail|p1a: Blissey` are the SNATCHER's; WRONG (pre-fix): the Snatch site ran the stolen move in the VICTIM's `Move(p2)` scope, so the step path's outcome owner was p2 against line order's p1 and `core_events` refused the battle (`version parse != step … outcome OWNER: engine scope says Some(1), line order says Some(0)`). No byte, draw or reading changed — the owner is read only by the parse gates and the persisted record. Pursuit's strike, the other nest, now goes through the same helper (`BattleState::in_nested_move_scope`) | `core_events_test::a_snatch_stolen_moves_outcome_is_owned_by_the_snatcher` + `…::a_pursuit_strikes_outcome_is_owned_by_the_pursuer` (each FAILS with its site's helper call reverted) | SCOPE (each line's source-record scope) + OWNER (both viewers) + both parse gates (whole-battle and per transition) |
