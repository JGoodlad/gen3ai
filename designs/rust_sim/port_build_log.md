# The Rust Showdown port — build log (CLOSED rounds)

Moved out of `src/rust_sim/CLAUDE.md` on **2026-09-07**, verbatim and in original order.

Each entry is a **closed** coverage round or fix: the differential gate that was built, the bug it
found, and the pins that hold it. The port itself is done (e2e 220/220 STRICT), so none of this is
work in progress — it is the evidence behind the current parity claims, kept because a later reader
debugging one mechanic wants the round that modelled it.

`## Next steps (engine, not yet built)` is included at the end: it is a ROADMAP, not current state.

**The live rules, the module map, the harness invocations and the current gate status stay in
...
**HISTORICAL NOTE (superseded — CLOSED by ROUND 30, kept for the reasoning):** ROUND 30 reproduced
nothing (1000 battles across two fresh seeds, 0 errors / 0 drain timeouts), PROVED the "deadlock" is
the sim correctly refusing an illegal choice and waiting for a legal one (`[Invalid choice]` carries
no re-request by design — `probe_illegal_choice_park.js`), and found + fixed the ONE drain call site
round 27 left unchecked. **There is no bridge deadlock.** The note below is the original reasoning:
The 1200-battle randbats soak stopped advancing at
...
### ROUND 34 (FIX) — the Masquerain-mirror flip is a REAL PORT BUG: the turn-0 tie is DRAWN correctly and then THROWN AWAY by the framing re-emit

> **Numbering note:** assigned `34` deliberately to dodge a KNOWN collision — other agents are
> writing rounds concurrently in other worktrees, and this worktree's own preceding section is
> numbered "ROUND 30", which already collides. Whoever consolidates should renumber freely.

Round 30's finding 5 left the `sbd_msdd8698_b293` repro FAILING AND VISIBLE with the decision
explicitly open: a same-species **Masquerain mirror** at a speed-TIED lead where the turn-0
Intimidate block is PERMUTED (node emits `-ability p2a` first, the port `p1a` first). Two candidate
readings: **(i)** a genuine gap in `gen3_turn0_construction_v1`, or **(ii)** the benign ROUND-18
`perside-construction-speed-tie-mirror-of-flip` attribution class that the JS differ simply has no
key for. Round 30 correctly refused to allowlist it without settling which.

**VERDICT: (i) — a REAL PORT BUG, and it was NOT in the PRNG model.** The construction window
consumes the sim's draws exactly; the bug is one layer up, in the LOG RECONSTRUCTION that runs
after it.

**THE EVIDENCE (`harness/probe_r34_turn0_mirror_tie.js`, sim-as-oracle).** The probe instruments
`battle.prng.rng.next` BEFORE `>player` (so no construction draw escapes), replays the repro's exact
`>start` seed `[53222,51495,47060,3453]` + packed teams through the real `BattleStream`, and prints
every turn-0 draw with its seed-before and its CALL SITE, next to the port's
`POKESIM_PRNG_TRACE=1` stream for the identical `START` line. It carries four NON-VACUITY guards
(same species / both Intimidate / raw Speed EQUAL / 2 Intimidate lines actually emitted); any guard
failing exits 2 rather than passing silently. Result — **all 5 turn-0 draws are IDENTICAL, value and
seed-state**:

```
  #0 -> 2135641050  seed_before=53222,51495,47060,3453   battle-queue.js:283 BattleQueue.insertChoice <- BattleActions.switchIn
  #1 -> 162476446   seed_before=32587,19418,7282,54804    PRNG.shuffle <- Battle.speedSort <- Battle.eachEvent
  #2 -> 3720464964  seed_before=2479,12702,63633,51367    (ditto)
  #3 -> 1290892772  seed_before=56769,51780,1693,10150    (ditto)
  #4 -> 3439877211  seed_before=19697,30180,54249,6465    Battle.randomChance <- Battle.endTurn   (Quick Claw)
  port: the same 5 values at the same 5 seed states.
```

Draw **#0** is the whole story: `insertChoice`'s tie window. `resolveAction` gives both leads'
`runSwitch` order 101 / equal speed, so inserting the SECOND one finds `firstIndex=0`,
`lastIndex=null → list.length=1`, hence `index = battle.random(0, 2)`. With #0 = 2135641050 that is
`floor(2135641050·2/2³²) = 0` → `list.splice(0,0,p2)` → **queue `[p2, p1]` → p2's Intimidate first**.
The port's `insert_runswitch` computes the same window and its `random_range(0,2)` is the same
formula, so the port's queue is ALSO `[p2, p1]` and `turn_loop` genuinely RAN p2 first. The board was
right. **Only the emitted bytes were wrong.**

**ROOT CAUSE — a stale model in the re-emit.** `BridgeSession::new_construct_turn0` runs the window
with **logging OFF** (the framing must be emitted first, in `emit_framing`), then RECONSTRUCTS the
leads' switch-in ability lines from the post-construction board. That reconstruction
(`turn/helpers.rs::emit_switchin_ability_lines`) re-derived the fire order with
`leads.sort_by(|a,b| b.spe.cmp(&a.spe))` — "faster raw Speed first, a TIE keeps side order". That is
the model of the DRAW-FREE `run_start_switchins` path, and it is correct THERE. Under
`gen3_turn0_construction_v1` the tie is broken by a real PRNG draw, so **at a raw-Speed tie the
re-derivation is right only half the time**, always guessing p1-first. Everything downstream
(state, seed, the remaining 405 commands) was already byte-clean — which is exactly why only the
protocol-comparing external-consistency gate could see it, and why the seed/state goldens never did.
Note this is broader than the mirror: it mis-orders the `-weather` SET block at a MIXED
weather-setter tie too, where the emitted order and the resulting `field.weather` then CONTRADICT
each other.

**FIX (`gen3_turn0_construction_mirror_order_v1`, 3 files).** RECORD the resolved order instead of
re-deriving it. `BattleState::turn0_switchin_order: Option<Vec<usize>>` (state.rs) is written by
`run_turn0_construction` straight off the queue `insert_runswitch` built (step 2b, right after the
tie draw), and `emit_switchin_ability_lines` consumes it when present. `None` — the draw-free
`start_with_switchins` path the committed goldens use — keeps the old faster-first/side-order
fallback, which is what that path's STATE half also does, so every committed golden is untouched.
Observation-only in the strict sense: it draws nothing and changes no board state; it only stops the
emitter from guessing at something already decided.

**GATE 3 — the repro.** `node harness/gen_sim_bridge_diff.js --repro …/sbd_msdd8698_b293` goes from

```
[repro] DIVERGED side=p1 chunk=2 line=11 kind=perside
  expected: ...|-ability|p2a: Masquerain|Intimidate|boost
  got:      ...|-ability|p1a: Masquerain|Intimidate|boost
```

to `[repro] ALLOWLISTED-ONLY (return102-numeric-alias) — no unexplained divergence.` (that residual
is the pre-existing request-DISPLAY deferral, untouched). **NO allowlist key was added** — round 30's
instinct to leave it visible rather than quiet it was right, and the differ's allowlist stays
request-display-only.

**GATE 2 — REVERT-VERIFIED regression tests** (`tests/turn0_construction_test.rs`, +3 to 9 tests):
- `turn0_speed_tie_mirror_emits_the_intimidate_block_in_the_drawn_order` — the Masquerain mirror at
  **both** tie outcomes (seed `[1,2,3,4]` → `p2a,p1a`; seed `[3,2,3,4]` → `p1a,p2a`), read off the
  real `BridgeSession::new_construct_turn0` chunks. **BOTH are asserted on purpose:** the pre-fix
  code PASSES the p1-first seed, so a one-seed test would also pass on a hard-coded flip; only the
  PAIR pins the draw as the discriminator (plus an `assert_ne!` that the two seeds differ at all).
- `turn0_distinct_speed_emits_the_faster_lead_intimidate_first` — the control (Masquerain 156 vs
  Tauros 256, both Intimidate): no tie, order is pure Speed, `p2a` first. Fails a "fix" that just
  inverts side order.
- `turn0_weather_setter_tie_emits_the_lines_in_the_order_the_board_agrees_with` — the
  EMISSION↔STATE pin at a MIXED setter tie (Kyogre Drizzle vs Groudon Drought, spe 216 both): seed
  `[1,2,3,4]` → Drought line then Drizzle line, `field.weather == Rain`; seed `[3,2,3,4]` → the
  reverse, `Sun`. Pre-fix, seed `[1,2,3,4]` emitted Drizzle-then-Drought while the board said Rain —
  a stream contradicting itself.
Ground truth for all of these is the real sim via
`harness/probe_r34_mirror_order_groundtruth.js` (never source-reading). **Revert-verified:**
re-pointing the emitter at `None::<Vec<usize>>` makes the mirror test fail with
`left: ["p1a","p2a"] right: ["p2a","p1a"]` and the weather test fail, while the distinct-speed control
and all 6 pre-existing seed pins still pass — i.e. the tests bite exactly the tie path. Source
restored and re-run green.

**GATE 1 — suite + goldens.** `cargo test --release`: **574 passed, 0 failed** (isolated
`CARGO_TARGET_DIR=/tmp/pokesim_masq_build`). e2e golden md5 **`3155eb796cb4bf453c6053d769ba98e5`,
UNCHANGED** — as it must be, since the goldens seed at the POST-construction `initSeed` and so take
the `None` fallback.

**GATE 4 — fresh-seed byte-fuzz self-validation.** `ab_fuzz.js --mode random --protocol --battles
400 --master-seed 34340803` (a FRESH seed; state+seed goldens cannot catch emission-order bugs):
3 non-allowlisted divergences, kinds `{state:1, seed:1, firstmover:1}`, 158/400 dropped
`forced-unmodeled-move`, 372 distinct species / 238 distinct moves covered. To prove those are
PRE-EXISTING and not mine, the SAME master seed was re-run against a PRE-FIX `ab_replay` built into a
separate target dir: **the same 3 battle ids (`ab_4_4`, `ab_14_22`, `ab_15_5`), the same kind
histogram, and `battle.txt` byte-identical apart from the run-id comment line.** So the fix is
byte-NEUTRAL on the omniscient path (expected — that path never takes the construction branch), and
those 3 are the known `--mode random` out-of-scope tail, not a regression. The gate that actually
covers the changed path is the bridge differ: a full `--mode randbats --master-seed 777001 --persistent
--battles 400` re-run — the seed that PRODUCED the repro — finishes **`diverged:0, errored:0,
drain_timeouts:0`** (400/400 ended, 123 fully clean, 277 allowlisted, ALL
`return102-numeric-alias`; 660 battles/hr, no throughput regression). Battle **293** itself now lands
in `allowlisted/` with its first difference at chunk 3 (the request `return102` alias) instead of
chunk 2 line 11 — the Intimidate block is byte-equal.

**WHY IT SURVIVED THIS LONG.** It needs three things at once — a raw-seed (bridge) battle, a
speed-TIED lead, and BOTH leads carrying a switch-in ability that emits — and then it is wrong only
on the ~50% of ties that draw p2-first. Every gate that could have seen it was looking elsewhere:
the seed/state goldens compare seeds and boards (both correct here), `ab_fuzz` replays the
omniscient log from the POST-construction seed (so the construction branch never runs), and
`bridge_replay --ab` legitimately CANNOT know the order at a tie from a post-construction seed — which
is precisely why ROUND 18 classified this shape as a benign residual THERE. That classification is
still right for `bridge_replay` (no raw seed ⇒ genuinely unmeasurable), and it was still right to
refuse to copy it into `gen_sim_bridge_diff.js`, where the raw seed IS available and the order IS
determined.

**LESSON.** A "benign, unmeasurable" classification is scoped to the OBSERVER that cannot measure
it. `bridge_replay` can't see the turn-0 tie order; the raw-seed differ can. Porting the key across
that boundary would have converted a real engine bug into a permanently-silenced allowlist entry —
the exact vacuity failure ROUND 18's own review caught in a laxer draft. The second lesson is
...
either bug above — `--use-bridge=rust` training would hang the same way — so this needs a real
diagnosis, not a restart."* This round settles it. **Verdict: (a) HARNESS, on every count. Neither
bridge child ever deadlocks; both were correct throughout.**

**1. REPRODUCE — the park does NOT come back.** Fresh soaks on the round-27 code, gen3customgame
unless noted: two `--persistent` randbats runs (master seeds **20260803** / **777001**, 600 + 400
battles) plus `--mode trapping --trap-prob 1.0` (25 battles, 51 `trapped:true` frames), `--format
gen3ou --trap-prob 1.0` (120 battles) and three smaller smokes (40 / 15 battles). Across all of
them: **0 errors, 0 stalls, 0 drain timeouts**, a steady ~620-680 battles/hr, and both long drivers
held ~1.6% CPU with their repro counts climbing monotonically throughout — the exact OPPOSITE of the
historical "alive but IDLE, no new repros for 2 h" shape. The park does not reproduce, which is the
expected result: round 27 fixed its cause.
*(The 777001 run did surface ONE non-allowlisted divergence, unrelated to any stall — finding 5.)*

**2. WHY A SILENT CHILD IS NOT A DEADLOCK (`probe_illegal_choice_park.js`).** The mechanism is
`Side.emitChoiceError` (`sim/side.ts:510`), and it is not really an "escalation on repeat" — it is a
one-line branch on whether the refusal actually CHANGED the request:

```
updated  -> `[Unavailable choice]` + emitRequest(..., true)   // a RE-REQUEST: the client recovers
!updated -> `[Invalid choice]`     + NOTHING                  // the client must re-pick from the
                                                              // request it ALREADY holds
```

So the second attempt at a move the request already marks `disabled` (round 27's trace), or any
out-of-range choice, legitimately produces **no new `|request|` frame**. The probe drives that
deterministically: `CHOOSE p1 move 4` on a 2-move mon → node emits
`|error|[Invalid choice] Can't move: Your Dugtrio doesn't have a move 4`, **0 requests** — and then a
LEGAL retry immediately produces the turn plus 2 requests. **PARK-THEN-RECOVER: the child was waiting
for a legal choice, not wedged.** Any driver that blocks until a `|request|` arrives converts that
into an unbounded wait; that is the whole bug class, and it lives in the driver.

**3. THE ASYMMETRY (worth knowing for `--use-bridge`).** On the same input the **rust** bridge
...
`probe_switch_probe_accepted.js`). Round 27 put `assertDrained` on the START and per-decision drains
but left the **trapped-probe drain** (`requireRequest:false`) unchecked, and that call site rests on
an assumption that is **false**: that a switch probe against a `trapped`/`maybeTrapped` request is
always REJECTED. `sim/pokemon.ts::getRequestData` emits `maybeTrapped` only when `trapped !== true`,
and that covers TWO different situations:
  * a **HIDDEN trap** — `tryTrap(true)` set `trapped = 'hidden'` (truthy, not `true`);
    `Side.chooseSwitch` (`side.ts:968`) REJECTS → `|error|` + a re-request. ← the assumed case;
  * a **SPECULATIVE maybe** — `battle.ts:1730` runs `FoeMaybeTrapPokemon` for every ability the foe's
    SPECIES could have (so cancelling can't leak whether it traps). The mon is **not** trapped, so
    `side.ts:981` ACCEPTS the switch and the sim emits **nothing**.
PROBE-SETTLED on a constructed matchup — a **Sand Veil** Dugtrio (the species CAN have Arena Trap)
flags the foe `maybeTrapped:true` while leaving it free to switch: in **gen3ou** the probe is accepted
silently; in **gen3customgame** the speculative loop is skipped (`battle.ts:1742` — no
`obtainableabilities`), which is exactly why every default-format soak, including the 25-battle
trapping run above, showed zero. **So this was NOT the historical park** (that was gen3customgame),
but it is the same defect class on the last unchecked drain, and it costs two things: the drain burns
its **full 30 s cap** (× up to `SAFETY`=600 decisions in one battle), and the accepted switch is
already the side's COMMITTED choice, so the follow-up choice is bogus and `cmds` records a command
that never committed — a saved repro that no longer replays.
**FIX:** the discriminator is now **CONTENT, not timing** — a rejected probe always answers with
`|error|`, so `probeWasAccepted()` keys on that (and correctly calls "the probe completed the
boundary" an ACCEPTANCE); an accepted probe skips the follow-up choice and, when BOTH sides commit
that way, the post-boundary drain drops its `requireRequest`. `PROBE_MAX_MS` (750 ms, ~100× the
sim's synchronous refusal latency) only bounds the "nothing is coming" conclusion, and a false
positive is LOUD — it leaves the boundary incomplete, which the next drain reports as a stall with a
repro.
**BELT-AND-BRACES (not instead of the fix):** a per-battle wall-clock budget
(`gen3_simbridge_battle_budget_v1`, `BATTLE_BUDGET_MS` 300 s) closes the outer-loop gap round 27's
note named — `drainQuiescent`'s `maxMs` caps a SINGLE drain, never the loop — and the summary now
carries `switch_probes_accepted` / `drain_timeouts` / `drain_timeouts_by_tag`, so a child going quiet
ANYWHERE is a reported number instead of an invisible slowdown. `drain_timeouts` must stay 0.

**THE GATE** — `probe_switch_probe_accepted.js`, deterministic (fixed teams, fixed `>start` seed,
fixed chooseSeed, `--trap-prob 1.0`), hermetic (both children are the node bridge, so no `cargo
build`), and non-vacuous (it first asserts a `"maybeTrapped":true` request was actually reached).
**REVERT-VERIFIED:** restoring the old probe block takes the same battle from **7.0 s to 181 s** and
the gate exits 1. (Assertion 2 — "no choice was sent for an already-committed side" — did NOT fire
on that revert: the extra choice happened to land on the NEXT boundary and was legal, so it skewed
`cmds` SILENTLY rather than erroring. It is kept as a guard for the loud variant; assertion 1 is the
load-bearing one.) Four `--selftest` cases pin the content discriminator itself.

**5. A BONUS FINDING, DELIBERATELY NOT FIXED — the mirror Intimidate flip is BACK on the live
binary** (repro `harness/sim_bridge_diff_out/divergences/sbd_msdd8698_b293`, seed 777001 battle
293). A same-species **Masquerain mirror** (both Intimidate, speed-TIED lead): node emits the
turn-0 Intimidate block as `-ability p2a` / `-unboost p1a` / `-ability p1a` / `-unboost p2a`, the
port leads with `p1a`. It is a ONE-COMMAND DETERMINISTIC repro —
`node harness/gen_sim_bridge_diff.js --repro harness/sim_bridge_diff_out/divergences/sbd_msdd8698_b293`
replays all 406 cmds through both real bridges and reproduces it exactly. Same multiset, permuted —
**exactly ROUND 18's form (b)**, the
`perside-construction-speed-tie-mirror-of-flip` class that `bridge_replay.rs` classifies as a benign
construction residual. Two things make it worth a decision rather than a reflex:
  * `gen3_turn0_construction_v1` claims the port reproduces the turn-0 construction window
    bit-for-bit from the RAW seed, which should make a speed-tied lead byte-exact — so either this
    mirror shape is a genuine gap in that model, or it is the benign attribution ambiguity round 18
    settled and the differ simply doesn't carry the key.
  * `gen_sim_bridge_diff.js`'s allowlist covers only request-DISPLAY residuals, and porting round
    18's key here would need ALL of A1's strictness clauses (identical framing multiset,
    same-species via the `|switch|` roster, ident-consistent-with-slot) — round 18's own review
    caught a REAL mirror MIS-ATTRIBUTION hiding inside a laxer version of that check. **Adding it
    carelessly is how this gate goes vacuous**, so it is left FAILING and visible.
  **RESOLVED in ROUND 34: it is (i), a real port bug** — all 5 turn-0 draws are identical, the
  port's queue really did run p2 first, and the framing RE-EMIT re-derived the order with the
  draw-free "tie = side order" model. Fixed in the engine; NO allowlist key was added.

**NO PORT CHANGE.** Nothing under `src/` (rust) was touched — suite and the e2e golden md5
`3155eb796cb4bf453c6053d769ba98e5` are unchanged by construction. Post-fix sanity: 40 randbats
battles, 0 diverged / 0 errored / 0 drain timeouts, 684 battles/hr (no throughput regression).

**LESSON (the same one round 27 paid for, on a call site it missed):** when a harness waits on a
child, the bound belongs at EVERY wait, and the terminal condition must be derived from what the
protocol GUARANTEES, not from what it usually does. "A trapped-switch probe is always rejected" was
true of the case that motivated the code and false of the case that hangs it — and the tell was
available for free in the sim source, one branch away from the code that sets the flag.
...
`writeline_test`, `trapping_test`) all pass; `src/rust_sim/harness/rust_record_replay_check.py` node-vs-rust
...
# Per-mechanic coverage rounds

*(Moved out of `src/rust_sim/CLAUDE.md` in the same 2026-09-07 pass, verbatim.)*
