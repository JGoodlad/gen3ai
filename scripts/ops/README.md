...
Every ENTRY POINT is `python -m main.ops.<name>` and prints its contract on `--help`. `run_ref`, `critic_readouts`, `critic_read_render` and `conditioning_meters` are LIBRARY modules — imported, never invoked — and have no `--help` of their own; their table rows say so.
