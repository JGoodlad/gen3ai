...
ILLEGAL_MOVE_REJECT_REASON = (
    ...
    "When an arm feeds an explicit move the request marks `disabled` (a Choice lock "
    "here), Node emits `|error|[Unavailable choice] Can't move: X's Y is disabled` + a "
    "re-request TO THAT SIDE ONLY, whose disabled slot gains `\"disabledSource\":\"\"` "
    "(Side.chooseMove's updateRequestForPokemon). The port's bridge.rs reject path "
    "handles only the trapped-SWITCH case, so it emits no |error|, no disabledSource, "
    "and re-opens the boundary with a request to BOTH sides. Reconciled ONLY when every "
    "LOG chunk is byte-equal on both sides (so the battle itself is identical); the "
    "arm's outcome / choices_used / requests are still compared strictly."
)
